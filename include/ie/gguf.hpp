...
public:
    ...
    // Resolve a pointer inside any shard's mmap to (fd, file offset) — the
    // pread fill path's addressing. False if p is outside every shard.
    bool locate(const void* p, int& fd, uint64_t& off) const noexcept;
