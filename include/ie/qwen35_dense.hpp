...
struct DsparkDrafter;   // src/model/dspark_drafter.hpp (M2 separate-draft spec)
...
// SoA-Q2 repacked weight (fast Q2_0 decode path, gemv_q2_0_soa.cpp). A canonical
// Prism ternary block_q2_0 [K,N] is repacked at load into per-column NATURAL-order
// streams (q2_qs 2-bit codes 4/byte, per-128 fp16 scale, ~2.125 bpw). When `qs !=
// nullptr` the dense forward routes this weight's decode GEMV through the int-dot
// W2A8 gemv_q2_0_soa_q8 and prefill through dequant_q2_0_soa_to_Bt + gemm_fp16.
// `dtype` tags which native format the streams hold: kQ2_0 (the default — 2-bit
// codes, qs = N*(K/4) bytes) or kQ1_0 (Prism 1-bit sign-only, native Phase B — qs
// = N*(K/8) bytes, same d plane). The dispatch (gemv_q2soa_T / the multi-bank
// fusion) branches on it so Q1-native rides gemv_q1_0_soa_q8 / dequant_q1_0_soa_to_Bt
// instead of the Q2 kernels. Default kQ2_0 keeps every existing site byte-identical.
struct Q2SoaW {
    uint8_t*  qs = nullptr;   // [N*(K/4)] Q2_0 or [N*(K/8)] Q1_0 codes, col-contig
    uint16_t* d  = nullptr;   // [N*(K/128)] per-128 fp16 scale (raw bits)
    uint32_t  K = 0, N = 0;
    DType     dtype = DType::kQ2_0;   // kQ2_0 (default) or kQ1_0 (native Phase B)
    bool active() const noexcept { return qs != nullptr; }
};
...
struct Qwen35LayerWeights {
    ...
    // SoA-Q2 fast-decode mirror (set when the corresponding GGUF tensor is Q2_0,
    // the Prism ternary 2-bit format; AoS not loaded). One per Q2_0 decode GEMV
    // weight — for a full-Q2_0 GGUF (Ternary-Bonsai-27B) every projection uses one.
    Q2SoaW ffn_gate_q2soa, ffn_up_q2soa, ffn_down_q2soa;
    Q2SoaW attn_q_q2soa, ssm_out_q2soa, attn_v_q2soa, attn_k_q2soa, attn_output_q2soa;
    Q2SoaW attn_qkv_q2soa, attn_gate_q2soa;
...
struct MtpHead {
    ...
    // Draft-only vocabulary prefix (0 = full). The head GEMV is ~80% of draft
    // cost and the draft is a guess the target re-checks, so narrowing it trades
    // acceptance for bandwidth with ZERO effect on emitted text.
    // IE_QWEN35_DRAFT_VOCAB=N.
    uint32_t draft_vocab = 0;

    // Optional Q8_0-SoA lm_head override (2026-08-16): the draft's per-step
    // full-vocab head GEMV over the private F16 copy (~2.5 GB read) was ~80% of
    // draft cost. When the owning model's lm_head is Q8_0-SoA packed ON THE SAME
    // DEVICE as this head, borrow it (half the bytes, int-dot lane). Drafts only
    // PROPOSE — verify decides acceptance — so this cannot affect output.
    const int8_t*   lm_q8_qs = nullptr;
    const uint16_t* lm_q8_d  = nullptr;
    void*           d_act_q8 = nullptr;   // block_q8_1x [Kmax/32] draft activation
    void use_soa_lm_head(const int8_t* qs, const uint16_t* d) { lm_q8_qs = qs; lm_q8_d = d; }

    // Own Q8_0-SoA copies of the MTP block weights (2026-08-16): the GGUF ships
    // them Q8_0; storing SoA int8 instead of dequanted F16 halves every draft
    // GEMV's bytes (same int-dot lane as the main path). Null ⇒ that weight fell
    ...
    int32_t *d_pos1=nullptr, *d_tok1=nullptr, *d_draft_ids=nullptr;
    ...
    void embed_device(sycl::queue& q, const int32_t* tok);
    void build_x(sycl::queue& q, const sycl::half* h_src, int32_t e_tok);
    void build_x_device(sycl::queue& q, const sycl::half* h_src,
                        const int32_t* e_tok);
    ...
    // IE_SPEC_TREE_PROBE only: the draft's 2nd-choice token at step 0 of the
    // last host draft() call (tree-spec sizing measurement). -1 otherwise.
    int32_t top2_last = -1;

    // Fast draft (TP + split): argmax stays on device between autoregressive
    // steps; the K winning IDs make one device-to-host transfer at burst end.
    void draft_device_argmax(sycl::queue& q, const sycl::half* h_last, int32_t tn,
...
public:
    ...
    //   * taps_out != null → export the dspark 5-tap conditioning (M2): the
    //     residual stream ws_x_ AFTER each layer in set_tap_layers() is copied
    //     into taps_out as [n_capture, T, H] fp16 (capture order). Used by the
    //     separate-draft (dspark) spec loop; requires set_tap_layers() first.
    ...
    sycl::event forward(sycl::queue& q,
                        ...
                        sycl::half* taps_out = nullptr);

    // dspark tap-export config: which layer OUTPUTS (0-indexed, post-FFN residual)
    // forward() copies into taps_out when it is non-null. Set once at load from the
    // drafter's dspark.dspark.target_layers. Empty → no tap export ever.
    void set_tap_layers(std::vector<int32_t> ls) { tap_layers_ = std::move(ls); }
    ...
    // Speculative-decode round accounting (filled by the dspark path for the
    // --spec-draft stats line). tau = mean committed tokens per verify round.
    struct SpecStats {
        uint64_t rounds   = 0;   // verify rounds executed
        uint64_t drafted  = 0;   // draft tokens proposed  (rounds * block_size)
        uint64_t accepted = 0;   // draft tokens accepted  (sum of per-round n)
        uint64_t bonus    = 0;   // bonus tokens committed (one per round)
        double   tau      = 0.0; // (accepted + bonus) / rounds
    };
    ...
    // ---- dspark separate-draft speculative decode (M2, single-GPU, GREEDY) ----
    // Same lossless draft→verify→accept→commit→emit machinery as spec_generate
    // (shared via spec_loop_), but the drafts come from a target-conditioned
    // DsparkDrafter instead of the native MTP head. The caller has prefilled the
    // prompt into kv/dn AND seeded the cumulative target-tap conditioning:
    //   feat: [rows * n_embd_cap] f32, rows = start_pos, positions start_pos..
    //         (i.e. the taps of every committed position 0..start_pos-1, capture
    //          order per row) — grown in place each round from the accepted prefix.
    //   pos:  [rows] absolute positions (0..start_pos-1).
    //   tn:   target argmax at the last prompt position (the round-1 anchor).
    // Emits up to max_new committed tokens through `emit` IN ORDER; byte-identical
    // to plain greedy decode. Block size (draft length) and n_capture come from the
    // drafter; verify length = block_size + 1.
    std::string spec_generate_dspark(sycl::queue& q, KvCache& kv, DeltaNetState& dn,
                                     DsparkDrafter& drafter,
                                     std::vector<float>& feat,
                                     std::vector<int32_t>& pos,
                                     int32_t tn, uint32_t start_pos,
                                     uint32_t max_new, const SpecEmit& emit,
    ...
    sycl::event gemv_q4soa_T(sycl::queue& q, const sycl::half* A,
                             ...
                             const void* x_q8_pre = nullptr);
    // SoA-Q2 dispatch (mirrors gemv_q6soa_T): T==1 → quantize act to Q8_1 +
    // int-dot gemv_q2_0_soa_q8; T≥2 → dequant SoA-Q2→fp16 + gemm_fp16 (reuses the
    // q6soa_bt_/q6soa_c_ prefill scratch). Requires w.active().
    sycl::event gemv_q2soa_T(sycl::queue& q, const sycl::half* A,
                             const Q2SoaW& w, sycl::half* y, uint32_t T,
                             const void* x_q8_pre = nullptr);
...
private:
    ...
    Q2SoaW output_q2soa_;          // SoA-Q2 lm_head (when output is Q2_0)
    ...
    std::vector<int32_t> tap_layers_;   // dspark tap-export layer ids (M2); empty=off

    // Shared GREEDY draft→verify→accept→commit→emit loop behind both spec paths
    // (MTP + dspark). `draft_fn(tn, p, drafted)` fills the round's draft block
    // (>= verify_len-1 tokens); `cond_fn(accepted, p)` updates the provider's
    // next-round conditioning from the accepted prefix. `hidden_out`/`taps_out`
    // are the per-provider forward exports (either may be null). Lossless.
    std::string spec_loop_(sycl::queue& q, KvCache& kv, DeltaNetState& dn,
                           int32_t tn, uint32_t start_pos, uint32_t max_new,
                           uint32_t verify_len,
                           sycl::half* hidden_out, sycl::half* taps_out,
                           const std::function<std::string(int32_t, uint32_t,
                                                           std::vector<int32_t>&)>& draft_fn,
                           const std::function<std::string(uint32_t, uint32_t)>& cond_fn,
                           const SpecEmit& emit, SpecStats* stats = nullptr);
