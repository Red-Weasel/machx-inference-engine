...
private:
    ...
    struct Workspace {
        ...
        // Persistent host sources for EXL3 routing uploads. Their lifetime lets
        // the in-order queue consume the async copies without a per-layer wait.
        std::vector<int32_t>    moe_host_expert;
        std::vector<int32_t>    moe_host_token;
        std::vector<int32_t>    moe_host_identity;
        std::vector<sycl::half> moe_host_weight;
