...
struct QuantPtr {
    ...
    // Natural-order SoA-Q4_K streams (repack_q4_K_to_soa) for the coalesced-load
    // decode kernel gemv_q4_K_soa_q8_v2. Null unless the loader repacked this Q4_K
    // weight (IE_Q4K_SOA_V2, default ON). ADDITIVE — `p` (AoS blocks) stays valid
    // for prefill (gemv_q_T) and any non-v2 dispatch. Only the crown's 2-D dense
    // Q4_K attn q/k/v/o + DeltaNet qkv/gate projections are repacked; the n_dims==2
    // guard in maybe_upload_q4k_soa_v2 skips the 3-D MoE expert banks (own SoA path).
    void* soa_q4_q    = nullptr;   // nibble plane      [N*(K/2)]   uint8
    void* soa_q4_sc   = nullptr;   // per-32 s_raw      [N*(K/32)]  int8
    void* soa_q4_mn   = nullptr;   // per-32 m_raw      [N*(K/32)]  int8
    void* soa_q4_d    = nullptr;   // per-256 d (fp16)  [N*(K/256)] uint16
    void* soa_q4_dmin = nullptr;   // per-256 dmin(fp16)[N*(K/256)] uint16
    // Natural-order SoA-Q6_K streams (repack_q6_K_to_soa) for the coalesced-load
    // decode kernel gemv_q6_soa_q8 (the proven 27B ffn_down / dense Q6 fast path).
    // Null unless the loader repacked this Q6_K weight (IE_Q6K_SOA, default ON).
    // ADDITIVE — `p` (AoS blocks) stays valid for prefill (gemv_q_T) and any non-SoA
    // dispatch. Only the crown's 2-D dense Q6_K projections that decode via gemv_q_i8
    // (attn_qkv / attn_v) + the top-level lm_head (output) are repacked; the n_dims==2
    // guard in maybe_upload_q6k_soa skips the 3-D MoE expert banks (own SoA path).
    void* soa_q6_lo = nullptr;     // 4-bit lo plane    [N*(K/2)]   uint8
    void* soa_q6_hi = nullptr;     // 2-bit hi plane    [N*(K/4)]   uint8
    void* soa_q6_sc = nullptr;     // per-16 int8 scale [N*(K/16)]  int8
    void* soa_q6_d  = nullptr;     // per-256 d (fp16)  [N*(K/256)] uint16
};
...
private:
    ...
    // SoA-Q6_K streams for the lm_head (output_) when it is Q6_K and IE_Q6K_SOA is
    // on. Only the soa_q6_* fields are used (its .p is left null — output_ holds the
    // AoS blocks). Populated by maybe_upload_q6k_soa at load; null → AoS fallback.
    QuantPtr output_q6soa_;
