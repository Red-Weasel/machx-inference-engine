...
#include "ie/gguf.hpp"
#include "ie/deltanet_state.hpp"   // DeltaNetState — for the never-used dn_state() dummy
...
public:
    ...
    // gpt-oss has NO DeltaNet, so dev_has_dn() is always false and dn_state()
    // returns an unused, never-initialized dummy — present ONLY so the shared cache
    // template (which handles both DeltaNet and DeltaNet-free models) compiles.
    // Only the per-card KvCache is snapshotted/restored. Per-card KV is self-
    // contained: replicate-attn (Phase 1, <=16k) computes full attention on each
    // card; head-shard (Phase 2) holds each card's head slice — either way the
    // all-reduce is downstream of attention, so the snapshot is valid per card.
    ...
    DeltaNetState&  dn_state(uint32_t /*dev*/)                 { return dn_dummy_; }
    bool            dev_has_kv(uint32_t dev)    const          { return kv_[dev].ready(); }
    bool            dev_has_dn(uint32_t /*dev*/) const         { return false; }
...
private:
    ...
    DeltaNetState        dn_dummy_;                     // never inited (ready()=false); satisfies FleetPrefixCache's DN accessor set — gpt-oss has no DeltaNet
