// include/ie/glm5next.hpp — GLM-5.3-Flash (`glm5next`) model runtime.
//
...
// vs dense-FFN vs MoE — Glm5NextConfig::is_full_attn()/is_dense_layer(); the
...
//           files and IE_G5_DENSE_Q8=0).
struct Glm5DW {
    sycl::half* w  = nullptr;
    int8_t*     qs = nullptr;
    uint16_t*   d  = nullptr;
    explicit operator bool() const noexcept { return w || qs; }
};
...
struct Glm5NextLayer {
    ...
    Glm5DW f_a;                       // [H -> 128]   gate low-rank in
    Glm5DW f_b;                       // [128 -> 8192] gate low-rank out
    Glm5DW g_a;                       // [H -> 128]   output-gate low-rank in
    Glm5DW g_b;                       // [128 -> 8192] output-gate low-rank out
    Glm5DW beta;                      // [H -> 64] per-head delta step
    ...
    Glm5DW q_b;                       // [1536 -> 16384] (64 heads x 256)
    Glm5DW kv_a;                      // [H -> 512] (latent only — no rope part)
    ...
    // IE_G5_PIN_BANKS is on (load-time; page cache released as copied) —
...
public:
    ...
    std::string load(DeviceAllocator& alloc, const GgufReader& g,
                     const Glm5NextConfig& cfg, uint64_t vram_budget_bytes = 0,
                     uint32_t layer_lo = 0, uint32_t layer_hi = UINT32_MAX);

    // Exact load pass-1 projection: tensor metadata only, no allocator or weights.
    static std::string plan_device_weights(const GgufReader& g, const Glm5NextConfig& cfg,
                                          uint32_t lo, uint32_t hi, uint64_t& bytes);
    const Glm5NextConfig& config() const noexcept { return cfg_; }
    const Glm5NextLayer&  layer(uint32_t il) const { return layers_[il]; }

    // Globals (device).
    sycl::half* token_embd = nullptr;  // [vocab, H] GGUF-native row-gather
    Glm5DW      lm_head;               // [H -> vocab]
    float*      output_norm = nullptr; // [H]

    void set_cpu_threads(uint32_t n) noexcept { cpu_threads_ = n; }
    uint32_t cpu_threads() const noexcept { return cpu_threads_; }

    uint64_t device_bytes() const noexcept { return dev_bytes_; }
    void configure_server_memory(const Glm5MemoryPolicy& policy, uint64_t cache,
                                 uint64_t pin_cap, uint64_t staging) {
        server_memory_ = true; memory_policy_ = policy;
        server_cache_bytes_ = cache; server_pin_cap_ = pin_cap;
        memory_policy_.floor_gib += staging / 1073741824.0;
    }
    uint64_t pinned_bank_bytes() const noexcept { return pinned_bytes_; }
    uint64_t expert_cache_bytes() const noexcept { return expert_cache_bytes_; }
    ...
    // Pre-fill the expert cache from a usage profile (IE_G5_EPROFILE_OUT
    ...
    // -- expert-parallel decode (IE_G5_EP_DECODE=1, needs the shared P2P
    // context from IE_G5_EP=1) ---------------------------------------------
    ...
    std::string ep_enable(Glm5NextModel& peer, uint32_t parity);
    ...
    // kpool - 1 (2051 on the real file — glm5next_n_select in the reference);
    ...
    // q* CPU workers' core placement (G1/G5', 2026-09-02): both teams sit on
    // the P-cores in BOTH modes — serial stages measured 9.42 vs 8.32 tok/s
    // with the tail team on the E-cores, concurrent stages (pipelined draft)
    // 11.4-12.0 vs 10.6-11.1 (the E block is bandwidth-bound: 12 threads did
    // no better than 8). A caller that runs the stages concurrently declares
    // it here (the log states the mode and its measured reason); the E-core
    // placement stays reachable through IE_G5_CPU_CORES_A/B.
    void set_cpu_partition(bool on) noexcept { cpu_partition_ = on; }
...
private:
    std::string load_impl(DeviceAllocator* alloc, const GgufReader& g,
                          const Glm5NextConfig& cfg, uint64_t budget,
                          uint32_t lo, uint32_t hi, uint64_t* projected_bytes);
    void free_all();
    ...
    void mm(const sycl::half* A, const Glm5DW& W, uint32_t T,
    ...
    void mm_dual(const sycl::half* A, const Glm5DW& Wa, const Glm5DW& Wb,
    ...
    Glm5NextConfig   cfg_{};
    std::vector<Glm5NextLayer> layers_;
    std::vector<void*> owned_;
    std::vector<void*> owned_host_;          // pinned bank copies
    uint64_t dev_bytes_  = 0;
    uint64_t host_bytes_ = 0;
    uint64_t pinned_bytes_ = 0;
    bool server_memory_ = false;
    Glm5MemoryPolicy memory_policy_{};
    uint64_t server_cache_bytes_ = 0, server_pin_cap_ = 0, expert_cache_bytes_ = 0;
    uint32_t layer_lo_ = 0, layer_hi_ = 0;
    // Diagnostic (IE_G5_DUMP_WIDE): per-forward host staging of the residual
    ...
    std::vector<uint16_t> dmoe_h_;       // IE_G5_DUMP_MOE per-expert staging
    ...
    sycl::half* mla_kv16_ = nullptr;         // [MT, kv_lora]
    ...
    struct ECache {
        ...
        // [C] slot whose VRAM failed the 2 MiB page-alias self-test: two virtual
        // pages of one malloc_device allocation backed by ONE physical page, so
        // filling this slot also rewrites its partner's. Never chosen as a victim.
        std::vector<uint8_t>  slot_bad;
        std::vector<uint64_t> last_use;      // [C] LRU stamps
        // R9 prefetch (IE_G5_PREFETCH): a prefetched slot's fill has had no
    ...
    // -- P2 DSA sparse path (engages only when max_ctx_ > n_sel) ------------
    // Verified against modeling_glm5_next.py Glm5NextIndexer: pools of
    // index_kpool=4 CONSECUTIVE tokens, gated-softmax pooled indexer keys,
    // ReLU(q.k) scored and head-weighted, top (index_topk/index_kpool)=512
    // pools expanded back to 2048 raw tokens + the incomplete tail (<=3),
    // padded to index_topk + index_kpool - 1 = 2051.
    //
    // A pool is IMMUTABLE once its 4th token lands, and with one unpadded
    // sequence first_key==0, so pool bounds are fixed at [0..3], [4..7], ...
    // => build each pool key ONCE on the token that completes it and cache
    // THAT (64 B/token/layer) instead of the reference's packed per-token
    // [k|gate|valid] (514 B/token/layer, re-pooled every forward).
    sycl::half* pool_key_ = nullptr;      // [n_full][n_pools_][IHD] pooled keys
    sycl::half* idx_k_ = nullptr;         // [T][IHD]  this chunk's indexer keys
    sycl::half* idx_gate_ = nullptr;      // [T][IHD]  this chunk's gate scores
    sycl::half* idx_kroll_ = nullptr;     // [n_full][kpool][IHD] open pool
    sycl::half* idx_groll_ = nullptr;     // [n_full][kpool][IHD] open pool
    float* idx_q_ = nullptr;              // [T][n_idx_heads][IHD] fp32
    float* idx_w_ = nullptr;              // [T][n_idx_heads] raw weights_proj
    float* idx_scores_ = nullptr;         // [T][n_pools_]
    int32_t* idx_sel_ = nullptr;          // [T][select_k] chosen pool ids
    int32_t* idx_tok_ = nullptr;          // [T][n_sel_] chosen token ids, ASCENDING
    int32_t* idx_pos_ = nullptr;          // [T] query positions (topk causal mask)
    int32_t* idx_nv_ = nullptr;           // [T] valid prefix length of idx_tok_ rows
    float* idx_part_ = nullptr;           // [T][NH][8][LAT+2] split-K attention partials
    // Speculative rewind must restore the indexer's OPEN-POOL buffers too, or a
    // rejected draft's key/gate stays in the roll and the pool that later
    // completes is built from a token that was never committed (a wrong pool
    // key is a wrong SELECTION for the rest of the sequence). The KDA state and
    // kv_len_ were saved; these were not. 12 layers x 4 x 128 halves = 12 kB each.
    sycl::half* snap_kroll_ = nullptr;
    sycl::half* snap_groll_ = nullptr;
    uint64_t    roll_bytes_ = 0;          // [n_full][kpool][IHD] halves, per stage
    // Spec-verify capture of the indexer key/gate, one row per speculated
    // position ([n_full][kSpecMax][IHD]). A PARTIAL accept restores the
    // pre-verify roll, which does not contain the rows being committed; without
    // replaying them the next pool to complete reads stale slots for ACCEPTED
    // tokens and caches a wrong pool key (Codex review 2026-09-04). ~25 kB each.
    sycl::half* sv_idx_k_ = nullptr;
    sycl::half* sv_idx_g_ = nullptr;
    uint32_t n_pools_ = 0;                // ceil(max_ctx_ / kpool)
    ...
    Glm5NextModel* ep_peer_ = nullptr;
    ...
    float* ep_vacc_ = nullptr;           // IE_G5_EP_VERIFY: local recompute of the peer half
    ...
    struct EpReq {
        const Glm5NextLayer* w; uint32_t L, T, n_exp, npick;
        ...
        // IE_G5_EP_PUSH: x was already PUSHED into our ep_x16_ (P2P D2D
    ...
    std::string ep_partial(const Glm5NextLayer& w, uint32_t L, uint32_t T,
    ...
    uint64_t pin_sz_ = 0;
    uint32_t pin_idx_ = 0;
    // R9 prefetch: worker-owned staging (IE_G5_PREFETCH) — never shared with
    ...
    // IE_G5_PP_TILES: prefill grouped-tiles wave jobs (device + host staging)
    int32_t* pp_jobs_ = nullptr;
    std::vector<int32_t> pp_hjobs_;
    // IE_G5_PP_STREAM: prefill layer-streaming — double-buffered whole-layer
    ...
    struct FillReq {
        ...
        bool stream = false;   // IE_G5_PP_STREAM: whole-layer region load —
    ...
    struct FillThread {
        ...
        bool stop = false;
        Glm5NextModel* owner = nullptr;
        void start(Glm5NextModel* m);
        std::shared_future<sycl::event> push(FillReq&& r);
        ~FillThread();
    };
    FillThread fill_;
    // docs/glm53/CAMPAIGN_2026-09-02.md G2 — N fill LANES for the bounce
    // requests (expert misses at decode and at prefill): each lane owns two
    // pinned bounce buffers and preads a whole expert itself (no sub-pool),
    // then issues its H2D on the shared in-order copyq_; N experts are in
    // flight instead of one.  The single FillThread keeps the prefetch and
    // whole-layer stream requests, which have single-consumer rings.
    // IE_G5_FILL_LANES=<n> (default 8; 0 = the old one-at-a-time path).
    // Segment-queue form (the one-expert-per-lane form lost decode: at T = 1 a
    // layer has ~2 misses and the chain is latency-bound, so an expert must be
    // read by MANY threads at once, and several experts at once for prefill):
    // every request takes a pinned buffer from a ring of kBufs, is cut into
    // 2 MiB segments on a shared queue, and the worker that completes the last
    // segment issues the H2D and resolves the promise.
    struct FillLanes {
        static constexpr uint32_t kBufs = 16;
        static constexpr uint64_t kSeg  = 2ull << 20;
        struct Req {
            FillReq r;
            uint8_t* pin = nullptr;
            uint32_t buf = 0;
        ...
        struct Seg { std::shared_ptr<Req> req; int fd; uint64_t off; const uint8_t* src; uint64_t len; uint8_t* dst; };
        std::vector<std::thread> workers;
        std::mutex mu;
        std::condition_variable cv;
        std::deque<Seg> segs;
        bool stop = false;
        Glm5NextModel* m = nullptr;
        uint8_t* bufs[kBufs] = {};
        std::shared_future<sycl::event> buf_fut[kBufs];   // the last request on each buffer: its
                                                          // H2D event once issued (wait both before reuse)
        uint32_t buf_next = 0;
        std::mutex tmu;                     // the diagnostic time counters
        std::string start(Glm5NextModel* owner, uint32_t n, uint64_t pin_sz);
        std::shared_future<sycl::event> push(FillReq&& r);
        void shutdown() noexcept;
        bool running() const noexcept { return !workers.empty(); }
    };
    FillLanes lanes_;
...
public:
    ...
    // IE_G5_SLOT_VERIFY: regions compared at compute time / regions wrong
    uint64_t sv_checks_ = 0, sv_bad_ = 0;
    // R9 shadow-router counting (IE_G5_SHADOW_COUNT): layer-ahead proxy score.
    ...
    uint64_t pf_issued_ = 0, pf_used_ = 0;   // IE_G5_PREFETCH diagnostics
    ...
    // IE_G5_PPS_SYNC probe (numbers-invalid): per-layer stream decomposition —
    // get = block until the layer's H2Ds are SUBMITTED, wait = until LANDED.
    double t_pps_get = 0, t_pps_wait = 0;
    bool traced_first_ = false;   // IE_G5_TRACE_FIRST: one per-layer trace done
    ...
    // IE_G5_CPU_MISS: host Q8×Q4/Q5 path for decode misses (no 16 MiB H2D).
    std::vector<sycl::half> cpu_x16h_;
    std::vector<float> cpu_xf_, cpu_y_;
    // Ring of host/device accumulators for the q* CPU branch: the slot's async
    // H2D (cpu_acc_ev_) is waited on before the slot is reused, so the depth
    // bounds how far the host may run ahead of q without stalling (2 measured
    // ~3% slower than unbounded on decode-96; 4 restores it).
    static constexpr uint32_t kCpuAccRing = 4;
    std::vector<float> cpu_acc_ring_[kCpuAccRing];
    float* cpu_acc_dev_[kCpuAccRing] = {};
    sycl::event cpu_acc_ev_[kCpuAccRing];    // the async H2D that last read each ring slot
    uint32_t cpu_acc_sel_ = 0;
    double t_cpu_compute = 0, t_cpu_join = 0;   // q* CPU branch: worker time / main-thread wait
    // Decode host-time partition (the ~19 ms/token "unaccounted" hunt, 2026-09-04).
    // Every host wait on the decode path is timed, so the split is COUNTED,
    // not inferred by subtraction from the wall clock.
    double t_route_wait = 0;    // run_moe: e_d2h.wait() for the routing (GPU drain behind it)
    double t_final_wait = 0;    // forward_range: the trailing q.wait() (lm_head + everything)
    double t_wide_wait  = 0;    // forward_range: wide_out D2H wait (stage A only)
    ...
    struct CpuWorker {
        ...
        int core_lo = -1, core_hi = -1;   // affinity range (the OMP team inherits it under KMP_AFFINITY=disabled)
        const char* stage = "?";          // "A"/"B" for the log lines
        void start();
...
private:
    ...
    uint32_t cpu_threads_ = 0;
    bool cpu_partition_ = false;             // set_cpu_partition(): the caller runs the stages concurrently
    // Which stage hosts the MTP draft kit (blk.45 + nextn.* + lm_head/norm for
    // the head): the tail by default; IE_G5_MTP_STAGE=head puts it on the FIRST
    // stage so a pipelined-draft caller drafts on card 0 while card 1 runs the
    // real token (G5' lever 3, 2026-09-02). Set at load, read by init_runtime.
    bool mtp_here_ = false;
...
// Shipped FP32 router GEMV. Ordered load-ahead preserves each expert's FMA lattice.
sycl::event glm5_router_logits(sycl::queue& q, const float* x, const float* w,
                               float* rl, uint32_t T, uint32_t H, uint32_t E,
                               const std::vector<sycl::event>& deps = {});

// Bias-corrected top-k selection; weights use the unbiased sigmoids.
// Requires 0 < top_k <= experts. Ties select the lower expert index.
// Supports ragged/larger expert counts and stable all-zero-probability weights.
