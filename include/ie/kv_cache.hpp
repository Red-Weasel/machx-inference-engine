...
public:
    ...
    //
    // INT8 KV is supported: the int8 shadow + per-row scales are copied and
    // fp16 is regenerated via dequantize_kv_from_int8 (both copy directions).
    // src and dst must AGREE on use_int8; a fp16<->int8 mismatch returns an
    // error status string.  Both caches must have the same n_layers_full,
    // n_kv_heads, head_dim.
