...
public:
    ...
    // qwen3moe is DeltaNet-free (KV-only): dev_has_dn() is always false and dn_state()
    // returns an unused, never-initialized dummy so the shared cache template compiles.
    // Each card's KV is head-sharded (nkv kv-heads/card) but self-contained — the
    // all-reduce is downstream of attention, so the per-card snapshot is valid.
...
private:
    ...
    DeltaNetState                      dn_dummy_;  // never inited (ready()=false); satisfies FleetPrefixCache's DN accessors — qwen3moe has no DeltaNet
