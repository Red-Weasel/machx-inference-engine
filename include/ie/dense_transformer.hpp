...
struct DenseQuantPtr {
    ...
    // weight (IE_Q4K_SOA_V2). ADDITIVE — `p` (AoS blocks) stays valid for prefill.
    ...
    // decode kernel gemv_q6_soa_q8_v2. Null unless the loader repacked this Q6_K
    // weight (IE_Q6K_SOA_V2). ADDITIVE — `p` (AoS blocks) stays valid for prefill.
    // Used for the universal Q6_K lm_head (output / tied token_embd).
...
private:
    ...
    // SoA-Q6 streams for the Q6_K lm_head (output / tied token_embd), coalesced-load
    // decode via gemv_q6_soa_q8_v2 (IE_Q6K_SOA_V2, default ON). Null → AoS fallback.
    void*  output_soa_q6_lo_ = nullptr;  void* output_soa_q6_hi_ = nullptr;
    void*  output_soa_q6_sc_ = nullptr;  void* output_soa_q6_d_  = nullptr;
