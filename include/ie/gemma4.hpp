...
public:
    ...
    // ---- VRAM-guarded single-endpoint prompt/KV cache (task #6) ----
    // Gemma fits neither KvCache nor FleetPrefixCache (raw per-layer std::vector
    // with variable head geometry), so the snapshot lives IN the model. Single
    // endpoint = last-conversation snapshot (not a trie).
    //
    // snapshot_kv: capture the first `depth` positions of every layer's live KV
    // into COMPACT per-layer buffers (kv-major, pos-contiguous — no max_ctx
    // padding, so a depth-N snapshot is exactly N/max_ctx of the live KV) and
    // remember `tokens` (the ids [0,depth)) for the next turn's LCP match.
    // VRAM-GUARDED: frees any prior snapshot FIRST (so its VRAM is reusable),
    // then allocs; if ANY alloc returns null it frees the partials and returns a
    // message (no snapshot, no crash) — exactly like FleetPrefixCache's "skip
    // insert, don't crash" guard. Empty return = snapshot taken.
    std::string snapshot_kv(sycl::queue& q, uint32_t depth,
                            const std::vector<int32_t>& tokens);
    // Copy the first `depth` (clamped to snapshot_depth) cached positions back
    // into the live per-layer KV at their original offsets; returns the restored
    // depth (0 if no snapshot / depth==0). The caller continues the prefill from
    // pos=depth — exact because gemma KV is purely positional/additive and the
    // matched tokens are identical (same contract as gpt-oss KV restore).
    uint32_t restore_kv(sycl::queue& q, uint32_t depth);
    uint32_t snapshot_depth() const noexcept { return snap_depth_; }
    const std::vector<int32_t>& snapshot_tokens() const noexcept { return snap_tokens_; }
...
private:
    ...
    // VRAM-guarded single-endpoint KV snapshot (task #6). Compact per-layer
    // buffers: n_kv[L]*head_dim[L]*snap_depth_ halfs each, kv-major/pos-contiguous
    // (stride snap_depth_ per kv-head, vs max_ctx in the live cache). NOT tracked
    // in owned_ (owned_ is freed en masse; these are freed/realloced per turn) —
    // free_snapshot() releases them, called on re-snapshot AND in the dtor.
    std::vector<sycl::half*> snap_k_, snap_v_;
    uint32_t snap_depth_ = 0;
    std::vector<int32_t> snap_tokens_;
    void free_snapshot();
