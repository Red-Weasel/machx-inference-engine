...
int main(int argc, char** argv) {
    ...
    auto* d_suh = static_cast<sycl::half*>(fleet.dev(0).malloc(K * sizeof(sycl::half)));
    auto* d_svh = static_cast<sycl::half*>(fleet.dev(0).malloc(N * sizeof(sycl::half)));
    q.memcpy(d_codes, trellis.data(), trellis.size()).wait();
    {
        std::vector<uint16_t> suh16(K), svh16(N);
        for (uint32_t k = 0; k < K; ++k) suh16[k] = f2h(suh[k]);
        for (uint32_t n = 0; n < N; ++n) svh16[n] = f2h(svh[n]);
        q.memcpy(d_suh, suh16.data(), K * sizeof(uint16_t)).wait();
        q.memcpy(d_svh, svh16.data(), N * sizeof(uint16_t)).wait();
    }
    ...
    // ============ Test D — batched gemm_exl3 vs per-row forward loop ============
    // The CORRECTNESS GATE for the batched prefill kernel: gemm_exl3 (decode-once/
    // MAC-many) must match the per-row gemv_exl3_forward loop to cosine >=
    // 0.999999. The second case exercises partial T_TILE and KC tails together.
    {
        auto run_batched_case = [&](uint32_t T, uint32_t Kc, const char* label) {
            auto* d_AT = static_cast<sycl::half*>(
                fleet.dev(0).malloc(size_t(T) * Kc * sizeof(sycl::half)));
            auto* d_yB = static_cast<sycl::half*>(
                fleet.dev(0).malloc(size_t(T) * N * sizeof(sycl::half)));
            auto* d_yR = static_cast<sycl::half*>(
                fleet.dev(0).malloc(size_t(T) * N * sizeof(sycl::half)));
            std::vector<uint16_t> AT16(size_t(T) * Kc);
            for (auto& v : AT16) v = f2h(nd(rng));
            q.memcpy(d_AT, AT16.data(), AT16.size() * sizeof(uint16_t)).wait();

            // The oracle's trellis is ki-major, so its prefix is a valid smaller-K
            // matrix whenever Kc is a multiple of the 128-wide Hadamard block.
            for (uint32_t t = 0; t < T; ++t)
                ie::gemv_exl3_forward(q, d_AT + size_t(t) * Kc, d_codes, d_suh, d_svh,
                                      d_yR + size_t(t) * N, Kc, N, bits).wait();
            ie::gemm_exl3(q, d_AT, d_codes, d_suh, d_svh, d_yB, T, Kc, N, bits).wait();

            std::vector<uint16_t> yB16(size_t(T) * N), yR16(size_t(T) * N);
            q.memcpy(yB16.data(), d_yB, yB16.size() * sizeof(uint16_t)).wait();
            q.memcpy(yR16.data(), d_yR, yR16.size() * sizeof(uint16_t)).wait();
            std::vector<float> yB(yB16.size()), yR(yR16.size());
            for (size_t i = 0; i < yB.size(); ++i) {
                yB[i] = h2f(yB16[i]);
                yR[i] = h2f(yR16[i]);
            }
            const double cosAll = cosine(yB, yR);
            const double errAll = maxabs_diff(yB, yR);
            double minRow = 1.0;
            for (uint32_t t = 0; t < T; ++t) {
                const auto b0 = yB.begin() + size_t(t) * N;
                const auto r0 = yR.begin() + size_t(t) * N;
                minRow = std::min(minRow,
                                  cosine(std::vector<float>(b0, b0 + N),
                                         std::vector<float>(r0, r0 + N)));
            }
            const bool pass = cosAll >= 0.999999 && minRow >= 0.99999;
            std::printf("[D:%s] batched gemm  T=%u K=%u  cosine=%.7f  min-row=%.7f  "
                        "maxerr=%.4e  %s\n",
                        label, T, Kc, cosAll, minRow, errAll, pass ? "PASS" : "FAIL");
            if (!pass) rc = 1;
            fleet.dev(0).free(d_AT);
            fleet.dev(0).free(d_yB);
            fleet.dev(0).free(d_yR);
        };

        run_batched_case(200, K, "full-k");
        if (K >= 1152)
            run_batched_case(65, 1152, "partial-k");
        else {
            std::printf("[D:partial-k] SKIP (oracle K=%u < 1152)\n", K);
            rc = 1;
        }
    }
    ...
    fleet.dev(0).free(d_suh); fleet.dev(0).free(d_svh);
