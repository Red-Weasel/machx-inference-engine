...
int main(int argc, char** argv) {
    ...
    // Buffer allocation
    // + 16: the XMX decode kernel zeroes the 15 rows after each appended token
    // and needs them inside the allocation (start_pos + 16 <= max_ctx), else
    // it falls back to SIMD16 and the top ctx point measures a mix.
    uint32_t max_ctx = *std::max_element(ctx_list.begin(), ctx_list.end())
                     + n_warmup + n_steps + 8 + 16;
