...
int main(int argc, char** argv) {
    ...
    // 27B dense-hybrid layer-split PPL gate: qwen35 across >1 cards uses
    // Qwen35SplitModel (host-in/host-out, owns its own per-card KV+DN) — the
    // only quality harness for the 2-card Q8_0 path (single-GPU Q8 host-spills).
    const bool is_q35_split = is_q35 && n_gpus > 1;
    ...
    ie::Qwen35SplitModel    q35split;    // 27B dense-hybrid layer-split (only when is_q35_split)
    ...
    if (is_dense) {
        ...
        if (is_q35_split) {
            ...
            ie::LayerPlan plan = ie::LayerPlan::contiguous(qcfg.n_transformer_layers(), n_gpus);
            if (auto err = q35split.load(fleet, plan, g, qcfg, max_ctx, int8_kv); !err.empty()) {
        ...
        if (auto err = q35ms.load(fleet, plan, g, cfg, max_ctx, int8_kv); !err.empty()) {
    ...
    ie::KvCache kv;
    {
        ...
        // TP / crown-split / 27B-split models own their per-card KV; the standalone kv is unused.
        if (!is_gptoss_tp && !is_q35moe_split && !is_q35_split) {
    ...
    ie::DeltaNetState dn;
    if (is_q35 && !is_q35_split) {   // split owns its own per-card DN
    ...
    if (!is_gptoss_tp && !is_q35moe_split && !is_q35_split) {   // TP / split lazily size their own per-card workspace + partials
    ...
    auto forward = [&](const int32_t* d_in, uint32_t T, uint32_t pos) -> sycl::event {
        ...
        if (is_q35_split) {
            if (tp_hids.size() < T) tp_hids.resize(T);
            q.memcpy(tp_hids.data(), d_in, T * sizeof(int32_t)).wait();
            if (auto m = q35split.forward(tp_hids.data(), T, pos, /*reset_kv=*/(pos == 0),
                                          tp_logits.data()); !m.empty())
                std::fprintf(stderr, "qwen35_split forward: %s\n", m.c_str());
            return q.memcpy(d_logits, tp_logits.data(), uint64_t(vocab) * sizeof(sycl::half));
        }
    ...
    if (!is_gptoss_tp && !is_q35moe_split && !is_q35_split) { kv.reset(); dn.reset(q); }   // TP/split reset their own KV via reset_kv=(pos==0)
