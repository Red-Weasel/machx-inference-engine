...
//        [--spec-head <mtp.gguf>] [--spec-draft <dspark.gguf>]
//        [--decode 128] [--runs 3] [--warmup 1]
...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        else if (a == "--spec-draft"  && i + 1 < argc) opts.spec_draft = argv[++i];
        ...
        else if (a == "--prefill-chunk" && i + 1 < argc)
            opts.prefill_chunk = uint32_t(std::max(1, std::atoi(argv[++i])));
    ...
    for (const auto& path : files) {
        ...
        for (uint32_t r = 0; r < runs; ++r) {
            ...
            // Lossless-gate instrumentation: dump the greedy text once per prompt
            // (spec vs non-spec runs must be byte-identical).
            if (r == 0 && std::getenv("IE_DUMP_TEXT"))
                std::fprintf(stderr, "[text] %s >>>%s<<<\n",
