...
void check(sycl::queue& q, uint32_t K, uint32_t N, bool unaligned, bool saturate) {
    ...
    std::vector<sycl::half> input(K), expected(2 * stride, sycl::half(-123));
    std::vector<ie::block_q4_K> weights(2 * blocks);
    ...
    for (unsigned projection = 0; projection < 2; ++projection) {
        for (uint32_t n = 0; n < N; ++n) {
            ...
            for (uint32_t b = 0; b < K / 256; ++b) {
                auto& w = weights[projection * blocks + size_t(n) * (K / 256) + b];
                ...
                for (unsigned j = 0; j < 8; ++j) {
                    scales[j] = saturate ? (projection ? 0 : 63) : (n * 7 + b * 3 + j * 11 + projection) % 64;
                    mins[j] = saturate ? (projection ? 63 : 0) : (n * 5 + b * 13 + j * 7 + projection * 9) % 64;
                }
                ...
                for (unsigned i = 0; i < 256; ++i) {
                    const unsigned quant = saturate ? 15 : (i * 7 + n * 3 + b * 11 + projection * 5) % 16;
                    w.qs[(i / 64) * 32 + i % 32] |= quant << ((i % 64 >= 32) ? 4 : 0);
                    const double value = double(d) * scales[i / 32] * quant - double(dm) * mins[i / 32];
                    sum += double(float(input[b * 256 + i])) * value;
                }
            }
            expected[projection * stride + 16 + n] = sycl::half(std::clamp(sum, -65504., 65504.));
        }
    }
    auto* allocation = sycl::aligned_alloc_device<sycl::half>(64, K + 1, q);
    auto* packed = sycl::malloc_device<ie::block_q4_K>(weights.size(), q);
    ...
    std::vector<sycl::event> deps{
        ...
        q.memcpy(packed, weights.data(), weights.size() * sizeof(ie::block_q4_K)),
        q.fill(output, sycl::half(-123), expected.size())};
    auto done = ie::gemv_q4_K_dual(q, a, packed, packed + blocks,
                                  output + 16, output + stride + 16,
                                  K, N, "q4_dual_dispatch_test", deps);
...
int main() {
    try {
        ...
        // Out-of-order by default: the returned event must cover both outputs.
        sycl::queue q(device);
        for (auto [K, N] : {std::pair{256u, 1024u}, {768u, 1025u}, {4096u, 2048u},
                            {8192u, 1024u}, {4096u, 1023u}, {8448u, 1024u},
                            {256u, 4097u}, {768u, 19u}})
            check(q, K, N, false, false);
        check(q, 4096, 2048, true, false);
        check(q, 4096, 2048, false, true);
        check(q, 4096, 2048, true, true);
        std::puts("ALL_EXACT");
        return 0;
