...
//     -m models/.seal/models/Qwen3.6-35B-A3B-Q4_K_M.gguf -p "<text>"
...
bool test_chatml_reasoning_preamble() {
    // Qwen3.8-style reasoning_effort sentence: rendered at the TOP of the system
    // block (vendor template order). Exact sentence comes from Engine::chat; the
    // builder only places whatever it is given.
    const std::string_view pre = "Reasoning effort is set to xhigh. Test sentence.";

    // 1. No system turn → standalone system block carrying only the sentence.
    std::vector<ie::ChatTurn> t1 = {{"user", "hi"}};
    const std::string p1 = ie::build_chatml_prompt(t1, true, true, {}, true, pre);
    const std::string e1 =
        "<|im_start|>system\nReasoning effort is set to xhigh. Test sentence.<|im_end|>\n"
        "<|im_start|>user\nhi<|im_end|>\n"
        "<|im_start|>assistant\n<think>\n";
    const bool ok1 = (p1 == e1);

    // 2. Leading system turn → merged under the sentence, blank line between.
    std::vector<ie::ChatTurn> t2 = {{"system", "Be brief."}, {"user", "hi"}};
    const std::string p2 = ie::build_chatml_prompt(t2, true, true, {}, true, pre);
    const std::string e2 =
        "<|im_start|>system\nReasoning effort is set to xhigh. Test sentence.\n\nBe brief.<|im_end|>\n"
        "<|im_start|>user\nhi<|im_end|>\n"
        "<|im_start|>assistant\n<think>\n";
    const bool ok2 = (p2 == e2);

    // 3. Empty preamble → byte-identical to the pre-existing render (no drift).
    const bool ok3 = ie::build_chatml_prompt(t2, true, true, {}, true) ==
                     ie::build_chatml_prompt(t2, true, true, {}, true, {});

    // 4. Tools branch → sentence first, then system content, then "# Tools".
    const char* mini_tools = R"([{"type":"function","function":{"name":"f","parameters":{}}}])";
    const std::string p4 = ie::build_chatml_prompt(t2, true, true, mini_tools, true, pre);
    const size_t i_pre = p4.find("Reasoning effort");
    const size_t i_sys = p4.find("Be brief.");
    const size_t i_tls = p4.find("# Tools");
    const bool ok4 = i_pre != std::string::npos && i_sys != std::string::npos &&
                     i_tls != std::string::npos && i_pre < i_sys && i_sys < i_tls;

    if (!ok1) std::fprintf(stderr, "  preamble standalone mismatch:\n    got: '%s'\n", p1.c_str());
    if (!ok2) std::fprintf(stderr, "  preamble merge mismatch:\n    got: '%s'\n", p2.c_str());
    char extra[96];
    std::snprintf(extra, sizeof(extra), " ok1=%d ok2=%d ok3=%d ok4=%d",
                  int(ok1), int(ok2), int(ok3), int(ok4));
    return report("ChatML reasoning-effort preamble placement", ok1 && ok2 && ok3 && ok4, extra);
}
...
// Gemma 4: the generation prompt must pre-fill an empty, closed "thought"
// channel after <|turn>model\n so the model (run with thinking disabled) skips
// its reasoning preamble. Without it the channel-name token "thought" (a NORMAL
// vocab token, not a special) leaks as literal text before the answer. The
// stable-prefix form (add_generation_prompt=false) must NOT carry the suffix.
bool test_gemma_prompt_golden() {
    std::vector<ie::ChatTurn> turns = {
        {"user", "What is the capital of France?"},
    };
    const std::string gen = ie::build_gemma_prompt(turns, /*add_generation_prompt=*/true);
    const std::string expected =
        "<|turn>user\nWhat is the capital of France?<turn|>\n"
        "<|turn>model\n"
        "<|channel>thought\n<channel|>";
    bool ok = (gen == expected);
    if (!ok) {
        std::fprintf(stderr, "  gemma gen-prompt mismatch:\n    got:      '%s'\n"
                             "    expected: '%s'\n", gen.c_str(), expected.c_str());
    }
    // Stable prefix (cache) must end at the closed user turn — no model marker,
    // no thought channel.
    const std::string stable = ie::build_gemma_prompt(turns, /*add_generation_prompt=*/false);
    const std::string expected_stable =
        "<|turn>user\nWhat is the capital of France?<turn|>\n";
    if (stable != expected_stable) {
        ok = false;
        std::fprintf(stderr, "  gemma stable-prefix mismatch:\n    got:      '%s'\n"
                             "    expected: '%s'\n", stable.c_str(), expected_stable.c_str());
    }
    return report("Gemma gen-prompt closed-thought-channel golden", ok);
}
...
int main(int argc, char** argv) {
    std::string gguf_path = "models/.seal/models/Qwen3.6-35B-A3B-Q4_K_M.gguf";
