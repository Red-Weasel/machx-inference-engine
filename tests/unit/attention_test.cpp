...
// --------------------------------------------------------------
// XMX split-K decode (full_attention_fa2_decode_xmx) vs the scalar reference
// AND vs the SIMD16 kernel, over every shape class the dispatcher can route
// there: gqa 1/4/8, head_dim 64/128/256, ctx lengths that end mid 16-key
// block (tail staging) / on a block edge / across several super-chunks.
// The cache is pre-filled with random rows; the kernel appends the last one.
// --------------------------------------------------------------
bool test_xmx_decode_vs_reference(sycl::queue& q, uint32_t n_q, uint32_t n_kv, uint32_t d,
                                  uint32_t start_pos, const char* label) {
    // Exactly 16 rows past start_pos exist: the kernel's minimum (it zeroes 15 of
    // them); every one of them starts NaN-poisoned below.
    const uint32_t max_ctx = start_pos + 16;
    std::mt19937 rng(0xC0DE + start_pos + d);
    std::uniform_real_distribution<float> dist(-0.5f, 0.5f);
    std::vector<float> hq(n_q * d), hk_in(n_kv * d), hv_in(n_kv * d);
    std::vector<float> hk(size_t(n_kv) * max_ctx * d, 0.f), hv(size_t(n_kv) * max_ctx * d, 0.f);
    for (auto& x : hq) x = dist(rng);
    for (auto& x : hk_in) x = dist(rng);
    for (auto& x : hv_in) x = dist(rng);
    // rows [0, start_pos) are history; the kernel appends row start_pos itself.
    for (uint32_t kv = 0; kv < n_kv; ++kv)
        for (uint32_t i = 0; i < start_pos; ++i)
            for (uint32_t dd = 0; dd < d; ++dd) {
                hk[(size_t(kv) * max_ctx + i) * d + dd] = dist(rng);
                hv[(size_t(kv) * max_ctx + i) * d + dd] = dist(rng);
            }
    for (uint32_t kv = 0; kv < n_kv; ++kv)
        for (uint32_t dd = 0; dd < d; ++dd) {
            hk[(size_t(kv) * max_ctx + start_pos) * d + dd] = hk_in[kv * d + dd];
            hv[(size_t(kv) * max_ctx + start_pos) * d + dd] = hv_in[kv * d + dd];
        }
    // Reference sees fp16-rounded inputs (what the kernel reads).
    auto r16 = [](std::vector<float>& v) { for (auto& x : v) x = float(sycl::half(x)); };
    r16(hq); r16(hk); r16(hv); r16(hk_in); r16(hv_in);
    std::vector<float> y_ref(n_q * d);
    attention_ref(1, n_q, n_kv, d, max_ctx, start_pos, hq, hk, hv, y_ref);
    ...
    auto* dq    = sycl::malloc_device<sycl::half>(n_q * d, q);
    auto* dk_in = sycl::malloc_device<sycl::half>(n_kv * d, q);
    auto* dv_in = sycl::malloc_device<sycl::half>(n_kv * d, q);
    auto* dk    = sycl::malloc_device<sycl::half>(size_t(n_kv) * max_ctx * d, q);
    auto* dv    = sycl::malloc_device<sycl::half>(size_t(n_kv) * max_ctx * d, q);
    auto* dy    = sycl::malloc_device<sycl::half>(n_q * d, q);
    auto* dy2   = sycl::malloc_device<sycl::half>(n_q * d, q);
    const uint32_t n_chunks = (max_ctx + 63u) / 64u;
    auto* parts = sycl::malloc_device<float>(size_t(n_chunks) * n_q * (d + 2), q);
    upload(dq, hq); upload(dk_in, hk_in); upload(dv_in, hv_in); upload(dk, hk); upload(dv, hv);
    // Poison the cache rows past ctx_len: the tail block must never read them.
    if (start_pos + 1 < max_ctx) {
        std::vector<sycl::half> nan_row(size_t(max_ctx - start_pos - 1) * d, sycl::half(std::numeric_limits<float>::quiet_NaN()));
        for (uint32_t kv = 0; kv < n_kv; ++kv) {
            q.memcpy(dk + (size_t(kv) * max_ctx + start_pos + 1) * d, nan_row.data(), nan_row.size() * sizeof(sycl::half)).wait();
            q.memcpy(dv + (size_t(kv) * max_ctx + start_pos + 1) * d, nan_row.data(), nan_row.size() * sizeof(sycl::half)).wait();
        }
    }
    ie::full_attention_fa2_decode_xmx(q, dq, dk_in, dv_in, dk, dv, dy, parts,
                                      start_pos, n_q, n_kv, d, max_ctx).wait();
    ie::full_attention_fa2_decode(q, dq, dk_in, dv_in, dk, dv, dy2, parts,
                                  start_pos, n_q, n_kv, d, max_ctx).wait();
    std::vector<sycl::half> hy(n_q * d), hy2(n_q * d);
    q.memcpy(hy.data(), dy, hy.size() * sizeof(sycl::half)).wait();
    q.memcpy(hy2.data(), dy2, hy2.size() * sizeof(sycl::half)).wait();
    sycl::free(dq, q); sycl::free(dk_in, q); sycl::free(dv_in, q); sycl::free(dk, q);
    sycl::free(dv, q); sycl::free(dy, q); sycl::free(dy2, q); sycl::free(parts, q);

    float max_abs = 0.f, max_abs_vs_simd = 0.f, max_ref = 0.f;
    bool finite = true;
    for (size_t i = 0; i < hy.size(); ++i) {
        const float a = y_ref[i], b = float(hy[i]), c = float(hy2[i]);
        if (!std::isfinite(b)) finite = false;
        max_abs = std::fmax(max_abs, std::fabs(a - b));
        max_abs_vs_simd = std::fmax(max_abs_vs_simd, std::fabs(b - c));
        max_ref = std::fmax(max_ref, std::fabs(a));
    }
    char extra[160];
    std::snprintf(extra, sizeof(extra), "  max|ref-xmx|=%.3g max|simd-xmx|=%.3g max|ref|=%.3g%s",
                  max_abs, max_abs_vs_simd, max_ref, finite ? "" : " NON-FINITE");
    // Same bound as the prefill gate (1e-2 abs vs the fp32 reference); the
    // SIMD16 kernel differs only by summation order (fp16 output rounding).
    return report(label, finite && max_abs <= 1e-2f && max_abs_vs_simd <= 4e-3f, extra);
}
...
int main() {
    ...
    // XMX decode: (n_q, n_kv, head_dim, start_pos) — gqa 4 / 8 / 1, hd 128 / 64 / 256,
    // ctx ending mid-block (tail staging), on a block edge, and across super-chunks.
    if (!test_xmx_decode_vs_reference(q, 32, 8, 128, 4095, "xmx decode gqa4 hd128 ctx 4096 (block edge)")) ++fails;
    if (!test_xmx_decode_vs_reference(q, 32, 8, 128, 4102, "xmx decode gqa4 hd128 ctx 4103 (tail block)")) ++fails;
    if (!test_xmx_decode_vs_reference(q, 64, 8, 128, 777,  "xmx decode gqa8 hd128 ctx 778")) ++fails;
    if (!test_xmx_decode_vs_reference(q, 8,  8, 128, 130,  "xmx decode gqa1 hd128 ctx 131")) ++fails;
    if (!test_xmx_decode_vs_reference(q, 16, 4, 64,  1000, "xmx decode gqa4 hd64 ctx 1001")) ++fails;
    if (!test_xmx_decode_vs_reference(q, 16, 2, 256, 300,  "xmx decode gqa8 hd256 ctx 301")) ++fails;
    if (!test_xmx_decode_vs_reference(q, 32, 8, 128, 0,    "xmx decode gqa4 hd128 ctx 1 (first token)")) ++fails;
    if (!test_xmx_decode_vs_reference(q, 32, 8, 128, 20000,"xmx decode gqa4 hd128 ctx 20001 (many supers)")) ++fails;
    if (!test_xmx_decode_vs_reference(q, 32, 8, 128, 2047, "xmx decode gqa4 hd128 ctx 2048 (block edge, poisoned +1..)")) ++fails;
