...
sycl::event sampling_penalties(sycl::queue& q,
                               ...
                               const std::vector<sycl::event>& deps) {
    if ((repeat == 1.0f && presence == 0.f && frequency == 0.f) || n_recent == 0)
        return q.submit([&](sycl::handler& h) { h.depends_on(deps); h.single_task([](){}); });
    return ie::ps(q, "sampling_penalties", [&](sycl::handler& h) {
        h.depends_on(deps);
        ...
        h.parallel_for(sycl::nd_range<1>(global, WG), [=](sycl::nd_item<1> it) {
            ...
            // One writer per ID: duplicated history entries must neither race
            // nor multiply the repetition/presence terms. Window <=512.
            for (uint32_t j = 0; j < i; ++j) if (recent_ids[j] == id) return;
            uint32_t count = 1;
            for (uint32_t j = i + 1; j < n_recent; ++j) count += recent_ids[j] == id;
            const float v = float(logits[id]);
            const float repeated = v > 0.f ? v / repeat : v * repeat;
            const float adjusted = repeated - presence - frequency * float(count);
            logits[id] = sycl::half(sycl::clamp(adjusted, -65504.f, 65504.f));
        });
    });
}
...
sycl::event repetition_penalty(sycl::queue& q,
                               ...
                               float penalty,
                               const std::vector<sycl::event>& deps) {
    return sampling_penalties(q, logits, vocab, recent_ids, n_recent, penalty, 0.f, 0.f, deps);
}
