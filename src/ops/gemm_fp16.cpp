// src/ops/gemm_fp16.cpp — FP16 GEMM via SYCL joint_matrix with SLM tiling.
//
// Reference: research/02_programming_stack.md §6.1.
//
// Layout
//   WG tile      BM × BN = 128 × 128
//   SG layout    4 × 4 subgroups (16 SGs/WG)
//   Per-SG tile  SGM × SGN = 32 × 32, decomposed into M_REPEAT=4 × N_REPEAT=2
//                XMX accumulators of shape TM=8 × TN=16
//   K tile       BK = 16 (one fp16 dpas worth)
//
// SLM
//   A_smem[BM × BK]   = 128 × 16 fp16 = 4 KiB
//   B_smem[BK × BN]   = 16 × 128 fp16 = 4 KiB
//   Single-buffered (no double-buffer in v1) — barrier between K iters.

#include "ie/ops.hpp"

#include <sycl/sycl.hpp>
#include <sycl/ext/oneapi/matrix/matrix.hpp>
#include "ie/kernel_profiler.hpp"

namespace ie {

namespace mat = sycl::ext::oneapi::experimental::matrix;
using fp16 = sycl::half;

namespace {

constexpr int SG_SIZE  = 16;
constexpr int BM       = 128;
constexpr int BN       = 128;
constexpr int BK       = 16;
constexpr int SGM      = 32;
constexpr int SGN      = 32;
constexpr int TM       = 8;
constexpr int TN       = 16;
constexpr int TK       = 16;
constexpr int M_REPEAT = SGM / TM;        // 4
constexpr int N_REPEAT = SGN / TN;        // 2
constexpr int SGS_PER_WG = (BM / SGM) * (BN / SGN);  // 16
constexpr int WG_ITEMS = SGS_PER_WG * SG_SIZE;       // 256

}  // namespace

static sycl::event gemm_fp16_large(sycl::queue& q,
                      const fp16* A, const fp16* B, float* C,
                      uint32_t M, uint32_t N, uint32_t K,
                      const std::vector<sycl::event>& deps) {
    // CONTRACT (read before passing ragged sizes):
    //   - K (contraction) may be ragged: the SLM loads zero-pad past K (guarded).
    //   - M, N may be ragged for the LOADS/compute, BUT the joint_matrix_store
    //     writes a FULL TM×TN (8×16) tile per accumulator and only the tile ORIGIN
    //     is bounds-checked. So a partial tail tile spills into the next C row (or
    //     past C). => the OUTPUT C must be allocated to tolerate writes up to
    //     round_up(M,TM) × round_up(N,TN) (i.e. pad C, or pass tile-aligned M/N).
    //   In-tree callers satisfy this: N,K are weight dims (tile-aligned, no
    //   column spill) and M is the token count (may be ragged, but the output C
    //   is sized for the max T → the row-tail spill lands in unused, recomputed
    //   rows). The danger case is a ragged N into a tightly-sized C (don't).
    const uint32_t wgs_m = (M + BM - 1) / BM;
    const uint32_t wgs_n = (N + BN - 1) / BN;

    return ie::ps(q, "gemm_fp16", [&](sycl::handler& h) {
        h.depends_on(deps);
        sycl::local_accessor<fp16, 2> A_smem({BM, BK}, h);
        sycl::local_accessor<fp16, 2> B_smem({BK, BN}, h);

        h.parallel_for(sycl::nd_range<2>({wgs_m * 1, wgs_n * WG_ITEMS},
                                         {1, WG_ITEMS}),
                       [=](sycl::nd_item<2> it) [[sycl::reqd_sub_group_size(SG_SIZE)]] {
            const int lid     = int(it.get_local_id(1));
            const int sg_id   = lid / SG_SIZE;            // 0..15
            const int sg_row  = sg_id / 4;                // 0..3
            const int sg_col  = sg_id % 4;                // 0..3
            const int wg_m    = int(it.get_group(0)) * BM;
            const int wg_n    = int(it.get_group(1)) * BN;
            const int sg_m0   = wg_m + sg_row * SGM;
            const int sg_n0   = wg_n + sg_col * SGN;
            auto sg = it.get_sub_group();

            // Accumulators: M_REPEAT × N_REPEAT
            mat::joint_matrix<sycl::sub_group, float, mat::use::accumulator,
                              TM, TN> acc[M_REPEAT][N_REPEAT];
            #pragma unroll
            for (int mr = 0; mr < M_REPEAT; ++mr)
                #pragma unroll
                for (int nr = 0; nr < N_REPEAT; ++nr)
                    mat::joint_matrix_fill(sg, acc[mr][nr], 0.0f);

            const int sg_lane = lid % SG_SIZE;        // 0..15
            for (int k = 0; k < int(K); k += BK) {
                // A load: 256 items × 8 fp16 each → 2048 = BM*BK.
                //   Map (lid) -> (row = lid/2, c_start = (lid%2)*8). Each item
                //   reads 8 contiguous fp16 from one row of A; subsequent lanes
                //   in the subgroup read consecutive 8-element strips.
                {
                    const int r       = lid / 2;
                    const int c_start = (lid & 1) * 8;
                    const uint64_t base = uint64_t(wg_m + r) * K + (k + c_start);
                    const bool row_ok = (wg_m + r) < int(M);
                    // Fast path: row in-bounds AND the whole 8-strip is within K
                    // (always true for K a multiple of 16 → zero overhead vs before).
                    if (row_ok && (k + c_start + 7) < int(K)) {
                        #pragma unroll
                        for (int dc = 0; dc < 8; ++dc) A_smem[r][c_start + dc] = A[base + dc];
                    } else {
                        // Ragged-K (or padded row): zero-fill columns past K so the
                        // dpas sees a clean zero tile instead of next-row garbage.
                        #pragma unroll
                        for (int dc = 0; dc < 8; ++dc) {
                            const bool kk_ok = (k + c_start + dc) < int(K);
                            A_smem[r][c_start + dc] = (row_ok && kk_ok) ? A[base + dc] : fp16(0);
                        }
                    }
                }
                // B load: 16 SGs × 16 lanes × 8 fp16 = 2048 = BK*BN.
                //   Map (sg_id) -> row, (sg_lane*8) -> c_start. Coalesced 16 B/lane.
                {
                    const int r       = sg_id;
                    const int c_start = sg_lane * 8;
                    const uint64_t base = uint64_t(k + r) * N + (wg_n + c_start);
                    const bool krow_ok = (k + r) < int(K);   // ragged-K row guard
                    if (krow_ok && wg_n + c_start + 7 < int(N)) {
                        #pragma unroll
                        for (int dc = 0; dc < 8; ++dc) B_smem[r][c_start + dc] = B[base + dc];
                    } else {
                        #pragma unroll
                        for (int dc = 0; dc < 8; ++dc) {
                            B_smem[r][c_start + dc] =
                                (krow_ok && wg_n + c_start + dc < int(N)) ? B[base + dc] : fp16(0);
                        }
                    }
                }
                sycl::group_barrier(it.get_group());

                // 4 × 2 = 8 mat_mads per SG.
                #pragma unroll
                for (int mr = 0; mr < M_REPEAT; ++mr) {
                    mat::joint_matrix<sycl::sub_group, fp16, mat::use::a,
                                      TM, TK, mat::layout::row_major> a_tile;
                    mat::joint_matrix_load(sg, a_tile,
                        A_smem.get_multi_ptr<sycl::access::decorated::no>() +
                        (sg_row * SGM + mr * TM) * BK,
                        /*stride=*/BK);
                    #pragma unroll
                    for (int nr = 0; nr < N_REPEAT; ++nr) {
                        mat::joint_matrix<sycl::sub_group, fp16, mat::use::b,
                                          TK, TN, mat::layout::row_major> b_tile;
                        mat::joint_matrix_load(sg, b_tile,
                            B_smem.get_multi_ptr<sycl::access::decorated::no>() +
                            sg_col * SGN + nr * TN,
                            /*stride=*/BN);
                        mat::joint_matrix_mad(sg, acc[mr][nr], a_tile, b_tile, acc[mr][nr]);
                    }
                }
                sycl::group_barrier(it.get_group());
            }

            // Store accumulators to global C.
            #pragma unroll
            for (int mr = 0; mr < M_REPEAT; ++mr) {
                #pragma unroll
                for (int nr = 0; nr < N_REPEAT; ++nr) {
                    const int row = sg_m0 + mr * TM;
                    const int col = sg_n0 + nr * TN;
                    if (row < int(M) && col < int(N)) {
                        mat::joint_matrix_store(sg, acc[mr][nr],
                            sycl::address_space_cast<sycl::access::address_space::global_space,
                                                     sycl::access::decorated::no>(
                                C + uint64_t(row) * N + col),
                            /*stride=*/N, mat::layout::row_major);
                    }
                }
            }
        });
    });
}

namespace {
template<int BM, int BN, int SGM>
sycl::event gemm_fp16_t(sycl::queue& q,
                      const fp16* A, const fp16* B, float* C,
                      uint32_t M, uint32_t N, uint32_t K,
                      const std::vector<sycl::event>& deps) {
    constexpr int SG_SIZE = 16, BK = 16, SGN = 32, TM = 8, TN = 16, TK = 16;
    constexpr int M_REPEAT = SGM / TM, N_REPEAT = SGN / TN;
    constexpr int WG_ITEMS = (BM / SGM) * (BN / SGN) * SG_SIZE;

    // The dispatcher supplies aligned N/K. As in the large kernel, stores
    // write complete 8-row tiles: C needs round_up(M,8) rows of storage.
    const uint32_t wgs_m = (M + BM - 1) / BM;
    const uint32_t wgs_n = (N + BN - 1) / BN;

    return ie::ps(q, "gemm_fp16", [&](sycl::handler& h) {
        h.depends_on(deps);
        sycl::local_accessor<fp16, 2> A_smem({BM, BK}, h);
        sycl::local_accessor<fp16, 2> B_smem({BK, BN}, h);

        h.parallel_for(sycl::nd_range<2>({wgs_m * 1, wgs_n * WG_ITEMS},
                                         {1, WG_ITEMS}),
                       [=](sycl::nd_item<2> it) [[sycl::reqd_sub_group_size(SG_SIZE)]] {
            const int lid     = int(it.get_local_id(1));
            const int sg_id   = lid / SG_SIZE;
            const int sg_row  = sg_id / (BN / SGN);
            const int sg_col  = sg_id % (BN / SGN);
            const int wg_m    = int(it.get_group(0)) * BM;
            const int wg_n    = int(it.get_group(1)) * BN;
            const int sg_m0   = wg_m + sg_row * SGM;
            const int sg_n0   = wg_n + sg_col * SGN;
            auto sg = it.get_sub_group();

            // Accumulators: M_REPEAT × N_REPEAT
            mat::joint_matrix<sycl::sub_group, float, mat::use::accumulator,
                              TM, TN> acc[M_REPEAT][N_REPEAT];
            #pragma unroll
            for (int mr = 0; mr < M_REPEAT; ++mr)
                #pragma unroll
                for (int nr = 0; nr < N_REPEAT; ++nr)
                    mat::joint_matrix_fill(sg, acc[mr][nr], 0.0f);

            for (int k = 0; k < int(K); k += BK) {
                #pragma unroll
                for (int z = lid * 8; z < BM * BK; z += WG_ITEMS * 8) {
                    const int r = z / BK, c = z % BK;
                    const uint64_t base = uint64_t(wg_m + r) * K + k + c;
                    const bool row_ok = wg_m + r < int(M);
                    if (row_ok && k + c + 7 < int(K)) {
                        #pragma unroll
                        for (int u = 0; u < 8; ++u) A_smem[r][c + u] = A[base + u];
                    } else {
                        #pragma unroll
                        for (int u = 0; u < 8; ++u)
                            A_smem[r][c + u] = row_ok && k + c + u < int(K) ? A[base + u] : fp16(0);
                    }
                }
                #pragma unroll
                for (int z = lid * 8; z < BK * BN; z += WG_ITEMS * 8) {
                    const int r = z / BN, c = z % BN;
                    const uint64_t base = uint64_t(k + r) * N + wg_n + c;
                    if (k + r < int(K) && wg_n + c + 7 < int(N)) {
                        #pragma unroll
                        for (int u = 0; u < 8; ++u) B_smem[r][c + u] = B[base + u];
                    } else {
                        #pragma unroll
                        for (int u = 0; u < 8; ++u)
                            B_smem[r][c + u] = k + r < int(K) && wg_n + c + u < int(N) ? B[base + u] : fp16(0);
                    }
                }
                sycl::group_barrier(it.get_group());

                // Each subgroup owns M_REPEAT × N_REPEAT accumulators.
                #pragma unroll
                for (int mr = 0; mr < M_REPEAT; ++mr) {
                    mat::joint_matrix<sycl::sub_group, fp16, mat::use::a,
                                      TM, TK, mat::layout::row_major> a_tile;
                    mat::joint_matrix_load(sg, a_tile,
                        A_smem.get_multi_ptr<sycl::access::decorated::no>() +
                        (sg_row * SGM + mr * TM) * BK,
                        /*stride=*/BK);
                    #pragma unroll
                    for (int nr = 0; nr < N_REPEAT; ++nr) {
                        mat::joint_matrix<sycl::sub_group, fp16, mat::use::b,
                                          TK, TN, mat::layout::row_major> b_tile;
                        mat::joint_matrix_load(sg, b_tile,
                            B_smem.get_multi_ptr<sycl::access::decorated::no>() +
                            sg_col * SGN + nr * TN,
                            /*stride=*/BN);
                        mat::joint_matrix_mad(sg, acc[mr][nr], a_tile, b_tile, acc[mr][nr]);
                    }
                }
                sycl::group_barrier(it.get_group());
            }

            // Store accumulators to global C.
            #pragma unroll
            for (int mr = 0; mr < M_REPEAT; ++mr) {
                #pragma unroll
                for (int nr = 0; nr < N_REPEAT; ++nr) {
                    const int row = sg_m0 + mr * TM;
                    const int col = sg_n0 + nr * TN;
                    if (row < int(M) && col < int(N)) {
                        mat::joint_matrix_store(sg, acc[mr][nr],
                            sycl::address_space_cast<sycl::access::address_space::global_space,
                                                     sycl::access::decorated::no>(
                                C + uint64_t(row) * N + col),
                            /*stride=*/N, mat::layout::row_major);
                    }
                }
            }
        });
    });
}
}

// Small expert batches otherwise execute a mostly empty 128-row tile.
// Keep the original path for large batches and shapes outside the measured
// aligned range. N selects enough workgroups to occupy the GPU.
sycl::event gemm_fp16(sycl::queue& q, const fp16* A, const fp16* B, float* C,
                      uint32_t M, uint32_t N, uint32_t K,
                      const std::vector<sycl::event>& deps) {
    if (M > 0 && M <= 128 && N >= 2048 && N <= 4096 && N % 128 == 0 &&
        K >= 1024 && K <= 4096 && K % 16 == 0) {
        if (N == 2048 && M <= 96)
            return gemm_fp16_t<64, 64, 16>(q, A, B, C, M, N, K, deps);
        return gemm_fp16_t<64, 128, 16>(q, A, B, C, M, N, K, deps);
    }
    return gemm_fp16_large(q, A, B, C, M, N, K, deps);
}

}  // namespace ie
