...
// Q8_0 equivalent: block = fp16 scale + 32 int8 quants → y[i] = d * qs[i].
// One WG per token; lanes stride over `hidden` (re-reads the block scale per
// element — cheap, fully cached). hidden is a multiple of 32 (Q8_0 block size).
sycl::event embedding_lookup_q8_0(sycl::queue& q,
                                  ...
                                  const std::vector<sycl::event>& deps) {
    constexpr uint32_t WG = 64;
    const auto* W = static_cast<const block_q8_0*>(token_embd_q8_0);
    const uint32_t blocks_per_token = hidden / 32;

    return ie::ps(q, "embed_q8_0", [&](sycl::handler& h) {
        h.depends_on(deps);
        h.parallel_for(sycl::nd_range<2>({n_tokens, WG}, {1, WG}),
                       [=](sycl::nd_item<2> it) {
            const uint32_t t   = uint32_t(it.get_global_id(0));
            ...
            const int32_t  tok = token_ids[t];
            if (tok < 0) return;
            const block_q8_0* row = &W[uint64_t(tok) * blocks_per_token];
            for (uint32_t i = lid; i < hidden; i += WG) {
                const block_q8_0& blk = row[i >> 5];          // block = i / 32
                const float d = dev_fp16_to_fp32(blk.d);
                y[uint64_t(t) * hidden + i] = sycl::half(d * float(blk.qs[i & 31]));
            }
        });
    });
}

// Q2_0 equivalent (Prism ternary 2-bit): block = fp16 scale + 32 B qs (128 w/blk,
// 2 bits/w, natural order, LSB-first) → y[j] = (code-1)*d. One WG per token; lanes
// stride over `hidden`. hidden is a multiple of 128 (Q2_0 block size).
sycl::event embedding_lookup_q2_0(sycl::queue& q,
                                  ...
                                  const std::vector<sycl::event>& deps) {
    constexpr uint32_t WG = 64;
    const auto* W = static_cast<const block_q2_0*>(token_embd_q2_0);
    const uint32_t blocks_per_token = hidden / 128;

    return ie::ps(q, "embed_q2_0", [&](sycl::handler& h) {
        h.depends_on(deps);
        h.parallel_for(sycl::nd_range<2>({n_tokens, WG}, {1, WG}),
                       [=](sycl::nd_item<2> it) {
            const uint32_t t   = uint32_t(it.get_global_id(0));
            ...
            const int32_t  tok = token_ids[t];
            if (tok < 0) return;
            const block_q2_0* row = &W[uint64_t(tok) * blocks_per_token];
            for (uint32_t i = lid; i < hidden; i += WG) {
                const block_q2_0& blk = row[i >> 7];          // block = i / 128
                const float d = dev_fp16_to_fp32(blk.d);
                const uint32_t j = i & 127u;
                const uint8_t code = (blk.qs[j >> 2] >> ((j & 3u) * 2u)) & 0x03u;
                y[uint64_t(t) * hidden + i] = sycl::half((int(code) - 1) * d);
            }
        });
    });
}

// Q1_0 equivalent (Prism 1-bit sign-only): block = fp16 scale + 16 B qs (128 w/blk,
// 1 bit/w, natural order, LSB-first) → y[j] = (2*bit-1)*d. One WG per token; lanes
// stride over `hidden`. hidden is a multiple of 128 (Q1_0 block size). Native
// Phase B path — Phase A expands Q1_0→Q2_0 at load and rides embedding_lookup_q2_0.
sycl::event embedding_lookup_q1_0(sycl::queue& q,
                                  ...
                                  const std::vector<sycl::event>& deps) {
    constexpr uint32_t WG = 64;
    const auto* W = static_cast<const block_q1_0*>(token_embd_q1_0);
    const uint32_t blocks_per_token = hidden / 128;

    return ie::ps(q, "embed_q1_0", [&](sycl::handler& h) {
        h.depends_on(deps);
        h.parallel_for(sycl::nd_range<2>({n_tokens, WG}, {1, WG}),
                       [=](sycl::nd_item<2> it) {
            const uint32_t t   = uint32_t(it.get_global_id(0));
            ...
            const int32_t  tok = token_ids[t];
            if (tok < 0) return;
            const block_q1_0* row = &W[uint64_t(tok) * blocks_per_token];
            for (uint32_t i = lid; i < hidden; i += WG) {
                const block_q1_0& blk = row[i >> 7];          // block = i / 128
                const float d = dev_fp16_to_fp32(blk.d);
                const uint32_t j = i & 127u;
                const uint8_t bit = (blk.qs[j >> 3] >> (j & 7u)) & 0x01u;
                y[uint64_t(t) * hidden + i] = sycl::half((int(bit) * 2 - 1) * d);
            }
        });
    });
}
...
sycl::event embedding_lookup_f16(sycl::queue& q,
    ...
    return ie::ps(q, "embed_f16", [&](sycl::handler& h) {
        h.depends_on(deps);
        h.parallel_for(sycl::nd_range<2>({n_tokens, WG}, {1, WG}),
                       [=](sycl::nd_item<2> it) {
            const uint32_t t   = uint32_t(it.get_global_id(0));
            ...
            if (tok < 0) return;
            const sycl::half* row = embd + uint64_t(tok) * hidden;
            for (uint32_t i = lid; i < hidden; i += WG)
                y[uint64_t(t) * hidden + i] = row[i];
        });
    });
}
