...
// The SIMD16 split-K kernel (unchanged); full_attention_fa2_decode below
// dispatches between it and the XMX kernel.
static sycl::event fa2_decode_fp16_impl_(sycl::queue& q,
                                         const sycl::half* q_in,
                                         const sycl::half* k_in,
                                         const sycl::half* v_in,
                                         sycl::half* k_cache,
...
// ===========================================================================
// full_attention_fa2_decode_xmx — split-K flash-decoding on the XMX engine.
// ===========================================================================
//
// Same split-K / online-softmax algorithm, the SAME partials layout
// ([super][q_head][head_dim+2], n_super as the fp16 kernel computes it) and an
// identical combine pass, so callers' scratch sizing is unchanged. What differs
// is the partial pass:
//   * one SUBGROUP per (kv_head, super_chunk); the gqa query heads that share
//     this kv_head are the M rows of an [8 × head_dim] Q tile (rows >= gqa are
//     zero; gqa <= 8, larger gqa takes the fp16 kernel);
//   * per 16-key block: S[8×16] = Q · K^T on XMX with K^T loaded col_major
//     STRAIGHT FROM THE KV CACHE (head_dim/16 block loads of 16×16 halves, no
//     SLM staging), the online softmax over the 16 columns in SLM, then
//     O[8×head_dim] += P · V with V rows loaded row_major straight from the
//     cache; O lives in SLM (this compiler's joint_matrix_apply carries no row
//     coordinate, so the per-row alpha rescale happens there, as in the XMX
//     prefill kernel) and the rescale is skipped, subgroup-uniformly, on every
//     block where no row max moved.
//   * the append pass also ZEROES the 15 cache rows after the appended token,
//     so the last, partial 16-key block reads real rows below ctx_len and zeros
//     above it (K zeros give masked columns; V zeros multiply P == 0) — no SLM
//     staging, no read past the allocation (the gate requires start_pos + 16 <=
//     max_ctx). Rows past ctx_len are by contract not yet valid for anyone.
// The fp16 kernel is bounded by one reduce_over_group per key and 2-byte scalar
// cache loads into SLM (~40-78 GB/s measured); this one has no per-key
// reduction and issues 512 B block loads. Standalone A/B 2026-09-10 (B70, 32/8
// heads, hd 128, fp16 KV): partial pass 0.337 -> 0.155 ms at 4K, 0.491 -> 0.130
// at 8K, 0.865 -> 0.213 at 16K (2.2x / 3.8x / 4.1x; 315 GB/s of KV at 16K),
// max |y_fp16 - y_xmx| 1.5e-5 on outputs of magnitude 1.7e-2. Numerics: fp16
// inputs, fp32 accumulation like the fp16 kernel; not bit-identical (different
// summation order). Kill switch: IE_FA2_DECODE_XMX=0.
// The XMX launch is one subgroup per (kv_head, super_chunk). Supers are sized so
// the launch carries ~kFa2XmxTargetSubgroups subgroups: with 8 kv heads that is
// the SIMD16 kernel's 64-super cap (identical partials count); with 2 kv heads
// it is four times as many supers, which is what keeps 32 Xe cores busy. Never
// more supers than 64-key chunks, so the caller's partials scratch (sized per
// chunk) always holds them.
static constexpr uint32_t kFa2XmxTargetSubgroups = 512;
static inline uint32_t fa2_xmx_n_super(uint32_t ctx_len, uint32_t n_kv_heads) {
    const uint32_t n_chunks   = (ctx_len + 63) / 64;
    const uint32_t want_super = std::max<uint32_t>(1u, kFa2XmxTargetSubgroups / std::max<uint32_t>(n_kv_heads, 1u));
    const uint32_t per_wg     = std::max<uint32_t>(1u, (n_chunks + want_super - 1) / want_super);
    return (n_chunks + per_wg - 1) / per_wg;
}
...
sycl::event full_attention_fa2_decode_xmx(sycl::queue& q,
    ...
    // Shape gate — outside it the SIMD16 kernel runs (never recurses).
    if (gqa == 0 || gqa > TM || n_q_heads % n_kv_heads != 0 ||
        head_dim % TK != 0 || head_dim > 256 || start_pos + KB > max_ctx)
        return fa2_decode_fp16_impl_(q, q_in, k_in, v_in, k_cache, v_cache, y,
                                     partials_scratch, start_pos, n_q_heads, n_kv_heads,
                                     head_dim, max_ctx, deps, prof);
    const uint32_t ctx_len  = start_pos + 1;
    const uint32_t n_blocks = (ctx_len + KB - 1) / KB;
    // Partials layout is per super ([super][q_head][hd+2]); the super count is
    // sized for the XMX launch (see fa2_xmx_n_super) and never exceeds the
    // 64-key chunk count the caller's scratch is sized for.
    constexpr uint32_t Bc = 64;
    const uint32_t n_chunks = (ctx_len + Bc - 1) / Bc;
    const uint32_t n_super  = fa2_xmx_n_super(ctx_len, n_kv_heads);
    const uint32_t CHUNKS_PER_WG = (n_chunks + n_super - 1) / n_super;
    const uint32_t blocks_per_super = CHUNKS_PER_WG * (Bc / KB);
    const uint32_t hd = head_dim;
    const uint32_t ND = hd / TN;                      // O tiles per row block (<= 16)

    // 1. Append the token row AND zero the KB-1 rows after it (they are inside
    //    the allocation by the gate above): the last 16-key block of the partial
    //    pass then reads zeros above ctx_len instead of stale or uninitialised
    //    memory. Row 0 of the 16 is the real token (idx / (n_kv*hd) == 0).
    auto append_evt = ie::ps(q, "fa2_xmx_append", [&](sycl::handler& h) {
        h.depends_on(deps);
        const uint32_t row_elems = n_kv_heads * head_dim;
        const uint32_t total = KB * row_elems;
        constexpr uint32_t WG = 256;
        const uint32_t global = ((total + WG - 1) / WG) * WG;
        h.parallel_for(sycl::nd_range<1>(global, WG), [=](sycl::nd_item<1> it) {
            ...
            const uint64_t out_off = (uint64_t(kv) * max_ctx + start_pos + r) * head_dim + d;
            if (r == 0) {
                const uint64_t in_off = uint64_t(kv) * head_dim + d;
                k_cache[out_off] = k_in[in_off];
                v_cache[out_off] = v_in[in_off];
            } else {
                k_cache[out_off] = fp16(0);
                v_cache[out_off] = fp16(0);
            }
        });
    });

    // 2. Partial pass on XMX: one subgroup (== one WG of 16 lanes) per
    //    (kv_head, super_chunk).
    auto partial_evt = ie::ps(q, "fa2_xmx_partial", [&](sycl::handler& h) {
        h.depends_on(append_evt);
        ...
        h.parallel_for(sycl::nd_range<1>(uint64_t(n_kv_heads) * n_super * SG_SIZE, SG_SIZE),
                       [=](sycl::nd_item<1> it) [[sycl::reqd_sub_group_size(SG_SIZE)]] {
            const uint32_t g     = uint32_t(it.get_group(0));
            const uint32_t kv    = g / n_super;
            const uint32_t super = g % n_super;
            const uint32_t lane  = uint32_t(it.get_local_id(0));
            auto sg = it.get_sub_group();
...
// IE_FA2_DECODE_XMX=0 keeps every decode on the SIMD16 kernel (the A/B control).
static bool fa2_decode_xmx_on() {
    static const bool on = [] {
        const char* e = std::getenv("IE_FA2_DECODE_XMX");
        return !(e && *e == '0');
    }();
    return on;
}

// Dispatcher (the public entry every model calls): the XMX kernel for the
// shapes it was validated on (head_dim 128, 1 <= gqa <= 8 — Qwen3 dense and
// MoE, Qwen2 distills, Llama-3 class) once the launch is wide enough to fill
// the GPU, the SIMD16 kernel for everything else.
// The XMX kernel launches n_kv_heads x n_super subgroups of 16 lanes; with few
// kv heads at short context that is too little parallelism for 32 Xe cores
// and the SIMD16 kernel (gqa subgroups per WG) wins. Floor on the launch size,
// not the context. Whole-token measurements (2026-09-10, one B70): Qwen3-4B
// (8 kv) +17% at a 1.7K prompt = 216 subgroups, unchanged at ctx 1; the 1.5B
// distill (2 kv) -3.5% at 4K = 128 subgroups (64 supers x 2), -5.7% at 1.7K =
// 54. So 192: 8 kv heads from ~1.5K context, 4 kv from ~3K, 2 kv from ~6K.
// The kernel profiler cannot resolve crossovers below 4K (clock-ramp noise on
// microsecond kernels), so whole-token numbers set this.
static constexpr uint32_t kFa2DecodeXmxMinSubgroups = 192;
static inline bool fa2_decode_xmx_shape_ok(uint32_t start_pos, uint32_t n_q_heads,
                                           uint32_t n_kv_heads, uint32_t head_dim,
                                           uint32_t max_ctx) {
    const uint32_t gqa = n_kv_heads ? n_q_heads / n_kv_heads : 0;
    if (!(head_dim == 128 && gqa >= 1 && gqa <= 8 && n_q_heads % n_kv_heads == 0)) return false;
    if (start_pos + 16 > max_ctx) return false;            // the zeroed tail rows must exist
    return n_kv_heads * fa2_xmx_n_super(start_pos + 1, n_kv_heads) >= kFa2DecodeXmxMinSubgroups;
}
sycl::event full_attention_fa2_decode(sycl::queue& q,
                                      ...
                                      const std::vector<sycl::event>& deps,
                                      AttnProfileData* prof) {
    if (fa2_decode_xmx_on() && fa2_decode_xmx_shape_ok(start_pos, n_q_heads, n_kv_heads, head_dim, max_ctx))
        return full_attention_fa2_decode_xmx(q, q_in, k_in, v_in, k_cache, v_cache, y,
                                             partials_scratch, start_pos, n_q_heads,
                                             n_kv_heads, head_dim, max_ctx, deps, prof);
    return fa2_decode_fp16_impl_(q, q_in, k_in, v_in, k_cache, v_cache, y, partials_scratch,
                                 start_pos, n_q_heads, n_kv_heads, head_dim, max_ctx, deps, prof);
}
...
sycl::event full_attention_fa2_decode_vec(sycl::queue& q,
    ...
    auto partial_evt = ie::ps(q, "fa2_partial_fp16_vec", [&](sycl::handler& h) {
        ...
        h.parallel_for(sycl::nd_range<2>({uint64_t(n_kv_heads) * n_super_chunks, WG_ITEMS},
            ...
            // Sized to the worst case in the A/B range: NTH_KQ=4 → head_dim/4=64
            // at head_dim=256 (27B/Bonsai; Coder's 128 needs 32). Only [0,DPL_KQ)
            // is touched (runtime-bounded loops). [32]→[64] 2026-07: the old size
            // (head_dim≤128 worst case) OVERFLOWED at head_dim=256 with NTH_KQ=4;
            // NTH_KQ stays at the safe default 8 (DPL_KQ=32 at hd256, in bounds).
            float q_vals[64];
            #pragma unroll
