...
sycl::event gemv_q4_K_dual(sycl::queue& q,
    ...
    return ie::ps(q, prof_name, [&](sycl::handler& h) {
        ...
        h.parallel_for(sycl::nd_range<1>(n_wgs * WG_ITEMS, WG_ITEMS),
            ...
            if (lane == 0) {
                y_alpha[n] = sycl::half(sycl::clamp(a_acc, -65504.f, 65504.f));   // saturate, not inf (fp16 overflow: GLM L28 e6 gate 6.5e4 -> NaN, 2026-09-01)
                y_beta [n] = sycl::half(sycl::clamp(b_acc, -65504.f, 65504.f));
            }
        });
    });
}
...
// kernel. y[r*N + n]. Built for the glm5next prefill MoE gate/up (replaces the
...
sycl::event gemv_q4_K_rows(sycl::queue& q,
    ...
    for (uint32_t r0 = 0; r0 < R; r0 += RT) {
        ...
        last = ie::ps(q, "gemv_q4k_rows", [&](sycl::handler& h) {
            h.depends_on(d);
            h.parallel_for(sycl::nd_range<1>(n_wgs * WG_ITEMS, WG_ITEMS),
                           [=](sycl::nd_item<1> it) [[sycl::reqd_sub_group_size(SG_SIZE)]] {
                const uint32_t lid  = uint32_t(it.get_local_id(0));
                ...
                const block_q4_K* col = &W[uint64_t(n) * blocks_per_col];
