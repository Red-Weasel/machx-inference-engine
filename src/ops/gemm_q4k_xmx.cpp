...
// =============================================================================
// gemm_q4_K_xmx_grouped — the XMX body over a prefill tile-job table
// (2026-08-27). The per-expert-chunk XMX launches lost 35% to occupancy
// (10 WGs each, in-order queue); ONE launch over every (job, gate|up,
// N-tile) keeps the dpas efficiency AND fills the device. tile_jobs =
// [J x 4] int32 {x_row0, rows<=16, slot, y_row0} over F16 mega row buffers;
// weights live in the expert-cache slots (slot_base + slot*slot_bytes +
// gate_off|up_off). Math per (job, mat) is gemm_q4_K_xmx's verbatim.
// =============================================================================
sycl::event gemm_q4_K_xmx_grouped(sycl::queue& q,
    ...
    return ie::ps(q, "gemm_q4k_xmx_grp", [&](sycl::handler& h) {
        h.depends_on(deps);
        sycl::local_accessor<fp16, 2> A_smem({M_TILE_MAX, BK}, h);
        sycl::local_accessor<fp16, 2> B_smem({BK, BN}, h);
        sycl::local_accessor<float, 1> C_scratch(
        ...
        h.parallel_for(sycl::nd_range<1>(uint64_t(J) * 2 * wgs_n * WG_ITEMS, WG_ITEMS),
                       [=](sycl::nd_item<1> it) [[sycl::reqd_sub_group_size(SG_SIZE)]] {
            const uint32_t lid    = uint32_t(it.get_local_id(0));
            const uint32_t gid    = uint32_t(it.get_group(0));
            const uint32_t j      = gid / (2 * wgs_n);
            const uint32_t rem    = gid % (2 * wgs_n);
            const bool     is_up  = rem >= wgs_n;
            const uint32_t wgid   = is_up ? rem - wgs_n : rem;
...
// =============================================================================
// gemm_q2_0_soa_xmx — SoA-Q2 (Prism ternary 2-bit) prefill GEMM via XMX/DPAS.
// Clones the gemm_q4_K_xmx SLM+joint_matrix skeleton EXACTLY, but the
// cooperative B-stripe is filled from the SoA-Q2 streams (q2_qs 2-bit codes +
// q2_d per-128 fp16 scale) with the (code-1)*d expansion instead of a Q4_K
// block decode. Internal M-tiling (tk_base loop, the moe_prefill_proj pattern)
// covers the WHOLE [T,N] chunk in ONE launch — so the SLM dequant amortizes
// across up to M_TILE_MAX=16 rows per group and never materializes an fp16
// Bt[K,N] in DRAM (deletes the 53.8 GB/pass round-trip of the dequant→gemm
// path). y is written fp16 directly; fp16 DPAS accumulate (same numeric class
// as gemm_fp16 — PPL-gated, not bit-exact vs the scalar dequant path).
// =============================================================================
namespace {

// Dequant one BK=256 K-tile (= 2 consecutive Q2_0 128-elem blocks) of ONE
// output column into that column's stripe `dst` (stride = BN). `qs_col` /
// `d_col` are the column bases in the SoA-Q2 streams; `bkt` is the 256-elem
// K-tile index. uint4 wide loads (16 B, K%256==0 keeps every block 32 B / 16 B
// aligned). Per-element math is bit-identical to dequant_q2_0_soa_to_Bt_v2:
// value = (code-1)*d, code = 2 LSB-first bits, natural order.
inline void dequant_q2_tile_to_stripe(const uint8_t* qs_col,
                                      const uint16_t* d_col,
                                      uint32_t bkt, fp16* dst, int stride) {
    #pragma unroll
    for (int sb = 0; sb < 2; ++sb) {             // 2 Q2 blocks × 128 = 256 elems
        const uint32_t blk = bkt * 2u + uint32_t(sb);
        const auto* qs4 = reinterpret_cast<const sycl::uint4*>(
            qs_col + uint64_t(blk) * 32u);
        ...
        for (int w = 0; w < 8; ++w) {
            ...
            for (int j = 0; j < 16; ++j) {
                ...
                dst[uint64_t(base + w * 16 + j) * stride] =
                    fp16(float(int(code) - 1) * d2);
            }
        }
    }
}
...
sycl::event gemm_q2_0_soa_xmx(sycl::queue& q,
    ...
    const uint32_t blocks_per_col = K / BK;        // BK=256 K-tiles per column
    const uint32_t n_chunks       = N / BN;
    const uint64_t qs_stride      = uint64_t(K) / 4;    // bytes/col in q2_qs
    const uint64_t d_stride       = uint64_t(K) / 128;  // scales/col in q2_d

    return ie::ps(q, "gemm_q2soa_xmx", [&](sycl::handler& h) {
        h.depends_on(deps);
        sycl::local_accessor<fp16, 2> A_smem({M_TILE_MAX, BK}, h);
        sycl::local_accessor<fp16, 2> B_smem({BK, BN}, h);
        sycl::local_accessor<float, 1> C_scratch(
        ...
        h.parallel_for(sycl::nd_range<1>(uint64_t(n_chunks) * WG_ITEMS, WG_ITEMS),
                       [=](sycl::nd_item<1> it) [[sycl::reqd_sub_group_size(SG_SIZE)]] {
            const uint32_t lid   = uint32_t(it.get_local_id(0));
            const uint32_t wgid  = uint32_t(it.get_group(0));   // n-chunk
            const uint32_t sg_id = lid / SG_SIZE;               // 0..3 — N-tile
            const uint32_t lane  = lid % SG_SIZE;
            const uint32_t wg_n  = wgid * BN;
            auto sg = it.get_sub_group();

            // Internal M-loop: one launch covers the whole T-chunk. Each tk_base
            // group of ≤ M_TILE_MAX rows re-walks the column weights, but only
            // from L1/L2 (K/4 B/col) and into SLM — no fp16 Bt DRAM round-trip.
            for (uint32_t tk_base = 0; tk_base < T; tk_base += M_TILE_MAX) {
                const uint32_t M = sycl::min(uint32_t(M_TILE_MAX), T - tk_base);
                const uint32_t m_groups = (M + TM - 1) / TM;
                ...
                for (uint32_t b = 0; b < blocks_per_col; ++b) {
                    // Cooperative A load: [M, BK=256] fp16.
                    {
                        const uint32_t total = M * BK;
                        for (uint32_t i = lid; i < total; i += WG_ITEMS) {
                            const uint32_t m  = i / BK;
                            const uint32_t kk = i % BK;
                            A_smem[m][kk] =
                                A[uint64_t(tk_base + m) * K + b * BK + kk];
                        }
                    }
                    // Cooperative B dequant: 64 lanes, BN=64 columns; each lane
                    // expands its column's 256-elem K-tile into the SLM stripe.
                    {
                        const uint32_t n_local = lid;     // 0..63
                        if (n_local < BN) {
                            const uint32_t n_global = wg_n + n_local;
                            const uint8_t*  qs_col = q2_qs + uint64_t(n_global) * qs_stride;
                            const uint16_t* d_col  = q2_d  + uint64_t(n_global) * d_stride;
                            dequant_q2_tile_to_stripe(qs_col, d_col, b,
                                                      &B_smem[0][n_local], BN);
                        }
                    }
                    sycl::group_barrier(it.get_group());
                ...
                for (uint32_t r = 0; r < M; ++r) {
                    ...
                    const float v = C_scratch[
                        uint64_t(g) * N_TILES_PER_WG * TM * TN +
                        sg_id * TM * TN + rg * TN + lane_col];
                    y[uint64_t(tk_base + r) * N + (col0 + lane_col)] = fp16(v);
                }
                sycl::group_barrier(it.get_group());
            }
        });
...
// =============================================================================
// gemm_f16_grouped_tiles — the grouped tile-table XMX body over F16 slot
// slices (2026-08-27, mixed-bank prefill). The UD file's 2 Q5_K gate/up and
// 5 Q8_0 down banks are F16-dequantized at load ([K, N] row-major per
// expert), so those layers failed the raw-bank grouped guard and fell to the
// per-expert path (~7K submits + 210 ms busy per 1024-chunk). This is
// gemm_q4_K_xmx_grouped with the B dequant replaced by a straight
// cooperative copy from the slot, BK dropped to 128 (down piece K=EF=640 is
// not 256-divisible), and a `mats` count so one entry point serves the dual
// gate/up piece (mats=2: off0/out0=gate, off1/out1=up) and the single down
// piece (mats=1). fp32 accumulate, half store — the per-expert
// gemm_fp16+cast route's numeric class (not bit-identical: tile order);
// prefill gates (chunked PPL + run2) adjudicate.
// =============================================================================
sycl::event gemm_f16_grouped_tiles(sycl::queue& q,
                          ...
                          const std::vector<sycl::event>& deps) {
    constexpr int BKF = 128;                    // K tile (EF=640 = 5 x 128)
    constexpr int KT_PER_BLK = BKF / TK;        // 8
    if (K % BKF != 0 || N % BN != 0 || mats < 1 || mats > 2) return {};
    const uint32_t k_blocks = K / BKF;
    const uint32_t wgs_n = N / BN;

    return ie::ps(q, "gemm_f16_grp", [&](sycl::handler& h) {
        h.depends_on(deps);
        sycl::local_accessor<fp16, 2> A_smem({M_TILE_MAX, BKF}, h);
        sycl::local_accessor<fp16, 2> B_smem({BKF, BN}, h);
        sycl::local_accessor<float, 1> C_scratch(
            M_GROUPS_MAX * N_TILES_PER_WG * TM * TN, h);

        h.parallel_for(sycl::nd_range<1>(
                           uint64_t(J) * mats * wgs_n * WG_ITEMS, WG_ITEMS),
                       [=](sycl::nd_item<1> it) [[sycl::reqd_sub_group_size(SG_SIZE)]] {
            const uint32_t lid    = uint32_t(it.get_local_id(0));
            const uint32_t gid    = uint32_t(it.get_group(0));
            const uint32_t j      = gid / (mats * wgs_n);
            const uint32_t rem    = gid % (mats * wgs_n);
            const bool     is_m1  = rem >= wgs_n;
            const uint32_t wgid   = is_m1 ? rem - wgs_n : rem;
            const uint32_t sg_id  = lid / SG_SIZE;
            const uint32_t lane   = lid % SG_SIZE;
            const uint32_t wg_n   = wgid * BN;
            auto sg = it.get_sub_group();
            ...
            const uint32_t y0   = uint32_t(tile_jobs[j * 4 + 3]);
            const fp16* A = A_mega + uint64_t(x0) * K;
            const auto* Wf = reinterpret_cast<const fp16*>(
                slot_base + uint64_t(slot) * slot_bytes +
                (is_m1 ? off1 : off0));
            fp16* y = (is_m1 ? out1 : out0) + uint64_t(y0) * N;
            const uint32_t m_groups = (M + TM - 1) / TM;
            ...
            for (uint32_t b = 0; b < k_blocks; ++b) {
                {   // Cooperative A load: [M, BKF] fp16.
                    const uint32_t total = M * BKF;
                    for (uint32_t i = lid; i < total; i += WG_ITEMS) {
                        const uint32_t m = i / BKF;
                        const uint32_t kk = i % BKF;
                        A_smem[m][kk] = A[uint64_t(m) * K + b * BKF + kk];
                    }
                }
                {   // Cooperative B copy from the [K, N] slot slice: lanes
                    // walk n within a k-row -> coalesced 128B reads.
                    for (uint32_t i = lid; i < uint32_t(BKF) * BN; i += WG_ITEMS) {
                        const uint32_t kk = i / BN;
                        const uint32_t nl = i % BN;
                        B_smem[kk][nl] =
                            Wf[uint64_t(b * BKF + kk) * N + wg_n + nl];
                    }
                }
                sycl::group_barrier(it.get_group());
