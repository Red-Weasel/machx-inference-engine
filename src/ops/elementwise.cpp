...
sycl::event cast_qkv_split_fp16_to_fp32(sycl::queue& q,
                                        ...
                                        const std::vector<sycl::event>& deps) {
    // B70: measured shapes use four adjacent channels per lane.
    // These widths are multiples of four; other shapes retain the flat kernel.
    if (T >= 16 && T <= 1024 &&
        ((k_total == 1024 && v_total == 2048) ||
         (k_total == 2048 && (v_total == 4096 || v_total == 6144)))) {
        const uint64_t row_stride = uint64_t(2) * k_total + v_total;
        const uint64_t channels = std::max(k_total, v_total);
        constexpr uint64_t WG = 64;
        const uint64_t channel_threads = ((channels + 4 * WG - 1) / (4 * WG)) * WG;
        return ie::ps(q, "cast_qkv_split", [&](sycl::handler& h) {
            h.depends_on(deps);
            h.parallel_for(sycl::nd_range<2>({T, channel_threads}, {1, WG}),
                           [=](sycl::nd_item<2> it) {
                const uint64_t row = it.get_global_id(0);
                const uint64_t channel = 4 * it.get_global_id(1);
                const uint64_t src_row = row * row_stride;
                if (channel < k_total) {
                    sycl::vec<sycl::half, 4> q_values, k_values;
                    q_values.load(0, src + src_row + channel);
                    k_values.load(0, src + src_row + k_total + channel);
                    const uint64_t dst_offset = row * k_total + channel;
                    q_values.convert<float>().store(0, q_dst + dst_offset);
                    k_values.convert<float>().store(0, k_dst + dst_offset);
                }
                if (channel < v_total) {
                    sycl::vec<sycl::half, 4> v_values;
                    v_values.load(0, src + src_row + uint64_t(2) * k_total + channel);
                    v_values.convert<float>().store(0, v_dst + row * v_total + channel);
                }
            });
        });
    }
...
sycl::event repeat_interleave_heads(sycl::queue& q,
    ...
    // B70: measured shapes use four adjacent channels per lane.
    // These widths are multiples of four; other shapes retain the flat kernel.
    if (T >= 1 && T <= 1024 && n_in_heads == 16 &&
        (head_dim == 128 || head_dim == 256) && (repeat == 2 || repeat == 3)) {
        constexpr uint64_t WG = 64;
        const uint64_t channel_threads = ((uint64_t(head_dim) + 4 * WG - 1) / (4 * WG)) * WG;
        return ie::ps(q, "repeat_heads", [&](sycl::handler& h) {
            h.depends_on(deps);
            h.parallel_for(sycl::nd_range<3>({T, n_in_heads, channel_threads}, {1, 1, WG}),
                           [=](sycl::nd_item<3> it) {
                const uint64_t row = it.get_global_id(0);
                const uint64_t head = it.get_global_id(1);
                const uint64_t channel = 4 * it.get_global_id(2);
                if (channel >= head_dim) return;
                const uint64_t src_offset = (row * n_in_heads + head) * head_dim + channel;
                const uint64_t dst_row_head = row * n_in_heads * repeat;
                sycl::vec<float, 4> values;
                values.load(0, x + src_offset);
                for (uint64_t r = 0; r < repeat; ++r) {
                    const uint64_t dst_head = interleave ? head * repeat + r
                                                         : r * n_in_heads + head;
                    values.store(0, y + (dst_row_head + dst_head) * head_dim + channel);
                }
            });
        });
    }
...
sycl::event split_q_gate_per_head(sycl::queue& q,
    ...
    // These widths are multiples of four; other shapes retain the flat kernel.
    if (T >= 1 && T <= 1024 &&
        ((n_heads == 16 && (head_dim == 128 || head_dim == 256)) ||
         ((n_heads == 24 || n_heads == 32) && head_dim == 256))) {
        constexpr uint64_t WG = 64;
        const uint64_t rows = uint64_t(T) * n_heads;
        const uint64_t channel_threads = ((uint64_t(head_dim) + 4 * WG - 1) / (4 * WG)) * WG;
        return ie::ps(q, "split_q_gate", [&](sycl::handler& h) {
            h.depends_on(deps);
            h.parallel_for(sycl::nd_range<2>({rows, channel_threads}, {1, WG}),
                           [=](sycl::nd_item<2> it) {
                const uint64_t row = it.get_global_id(0);
                const uint64_t channel = 4 * it.get_global_id(1);
                if (channel >= head_dim) return;
                const uint64_t src_offset = row * 2 * head_dim + channel;
                const uint64_t dst_offset = row * head_dim + channel;
                sycl::vec<sycl::half, 4> q_values, gate_values;
                q_values.load(0, qk + src_offset);
                gate_values.load(0, qk + src_offset + head_dim);
                q_values.store(0, q_out + dst_offset);
                gate_values.store(0, gate_out + dst_offset);
            });
        });
    }
...
sycl::event rope_partial(sycl::queue& q,
    ...
    return ie::ps(q, "rope", [&](sycl::handler& h) {
        ...
        // Prefill in place only needs pair owners. Keep the small-grid
        // launch unchanged: fewer groups regress single-token/short-head cases.
        const bool pairs_only = x == y && n_tokens >= 16 && global_x >= 512;
        const uint32_t width = pairs_only ? n_rotary / 2 : head_dim;
        const uint64_t global_y = ((uint64_t(width) + WG - 1) / WG) * WG;
        if (!global_x || !global_y) { h.single_task([] {}); return; }
...
// Interleaved 3-stream M-RoPE (qwen4exp vision). rope_partial with the position
// selected per frequency index: owner(r) = r % 3 → T/H/W stream (exactly the
// [11,11,10] interleave at n_rotary 64 — docs/qwen4/14_mrope.md §2). positions
// is [3, n_tokens] stream-major. Equal rows ⇒ bit-identical to rope_partial.
sycl::event rope_imrope3(sycl::queue& q,
                         ...
                         float theta_base,
                         const std::vector<sycl::event>& deps) {
    return ie::ps(q, "rope_im3", [&](sycl::handler& h) {
        h.depends_on(deps);
        constexpr uint32_t WG = 64;
        const uint64_t global_x = uint64_t(n_tokens) * uint64_t(n_heads);
        // Prefill in place only needs pair owners. Keep the small-grid
        ...
        h.parallel_for(sycl::nd_range<2>({global_x, global_y}, {1, WG}),
            ...
            const sycl::half* xr = x + base_idx;
            sycl::half*       yr = y + base_idx;
            if (d >= n_rotary) { if (!pairs_only) yr[d] = xr[d]; return; }
            const uint32_t half = n_rotary / 2;
