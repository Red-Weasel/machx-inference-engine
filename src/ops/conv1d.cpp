...
//
// SiLU is fused into the convolution.
...
static sycl::event depthwise_conv1d_causal_impl(sycl::queue& q,
    ...
    auto compute_evt = ie::ps(q, "conv1d_compute", [&](sycl::handler& h) {
        h.depends_on(deps);
        ...
        h.parallel_for(sycl::nd_range<2>({global_t, global_c}, {1, WG_C}),
            ...
            if constexpr (std::is_same_v<Output, float>) {
                // Preserve the old half store -> float cast exactly. icpx 2026.1
                // eliminates an ordinary half round trip (including vec RTE).
                // A volatile private bit store keeps that rounding observable.
                volatile uint16_t rounded_bits = sycl::bit_cast<uint16_t>(sycl::half(silu));
                const uint16_t bits = rounded_bits;
                y[uint64_t(t) * channels + c] = float(sycl::bit_cast<sycl::half>(bits));
            } else {
    ...
    if (T < kernel - 1) {
        return ie::ps(q, "conv1d_state", [&](sycl::handler& h) {
...
sycl::event depthwise_conv1d_causal(sycl::queue& q,
                                    const sycl::half* x, const sycl::half* w,
                                    sycl::half* conv_state, sycl::half* y,
                                    uint32_t T, uint32_t channels, uint32_t kernel,
                                    const std::vector<sycl::event>& deps) {
    return depthwise_conv1d_causal_impl(q, x, w, conv_state, y, T, channels, kernel, deps);
}

sycl::event depthwise_conv1d_causal_f32(sycl::queue& q,
                                    const sycl::half* x, const sycl::half* w,
                                    sycl::half* conv_state, float* y,
                                    uint32_t T, uint32_t channels, uint32_t kernel,
                                    const std::vector<sycl::event>& deps) {
    return depthwise_conv1d_causal_impl(q, x, w, conv_state, y, T, channels, kernel, deps);
}
