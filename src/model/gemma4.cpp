...
Gemma4Model::~Gemma4Model() {
    if (!alloc_) return;
    free_snapshot();                       // snapshot buffers are NOT in owned_
    for (void* p : owned_) alloc_->free(p);
    owned_.clear();
}

void Gemma4Model::free_snapshot() {
    if (alloc_) {
        for (auto* p : snap_k_) if (p) alloc_->free(p);
        for (auto* p : snap_v_) if (p) alloc_->free(p);
    }
    snap_k_.clear();
    snap_v_.clear();
    snap_depth_ = 0;
...
std::string Gemma4Model::snapshot_kv(sycl::queue& q, uint32_t depth,
                                     const std::vector<int32_t>& tokens) {
    // Free-first: releases the prior snapshot's VRAM so the new alloc can reuse
    // it (fewer OOMs) AND leaves us with NO snapshot if the new alloc fails.
    free_snapshot();
    if (depth == 0) return {};
    if (kcache_.empty() || depth > kv_ctx_) return "gemma4 snapshot: depth exceeds kv_ctx";
    const uint32_t L = cfg_.n_layers;
    snap_k_.assign(L, nullptr);
    snap_v_.assign(L, nullptr);
    // Allocate the compact per-layer buffers. Any null → free partials and bail
    // (the VRAM guard: a full-depth snapshot ~doubles KV, which OOMs at long ctx;
    // here we skip gracefully instead of crashing).
    for (uint32_t l = 0; l < L; ++l) {
        const uint64_t n = uint64_t(layers_[l].n_kv) * layers_[l].head_dim * depth;
        auto* k = static_cast<sycl::half*>(alloc_->malloc(n * sizeof(sycl::half)));
        auto* v = static_cast<sycl::half*>(alloc_->malloc(n * sizeof(sycl::half)));
        if (!k || !v) {
            if (k) alloc_->free(k);
            if (v) alloc_->free(v);
            free_snapshot();
            return "gemma4 snapshot: VRAM alloc failed (skipped, no cache)";
        }
        snap_k_[l] = k;
        snap_v_[l] = v;
    }
    // Gather [0,depth) per kv-head from the live cache (stride max_ctx*head_dim,
    // element (kv,pos,d) at (kv*max_ctx+pos)*head_dim+d — attention.cpp) into the
    // compact snapshot (stride depth*head_dim). One kernel per layer, K+V together.
    const uint32_t MC = kv_ctx_;
    for (uint32_t l = 0; l < L; ++l) {
        const uint32_t nkv = layers_[l].n_kv, hd = layers_[l].head_dim;
        const sycl::half* ks = kcache_[l]; sycl::half* kd = snap_k_[l];
        const sycl::half* vs = vcache_[l]; sycl::half* vd = snap_v_[l];
        const uint64_t total = uint64_t(nkv) * depth * hd;
        q.parallel_for(sycl::range<1>(total), [=](sycl::id<1> i) {
            const uint64_t idx = i[0];
            const uint32_t d   = uint32_t(idx % hd);
            const uint32_t pos = uint32_t((idx / hd) % depth);
            const uint32_t kv  = uint32_t(idx / (uint64_t(hd) * depth));
            const uint64_t src = (uint64_t(kv) * MC + pos) * hd + d;
            kd[idx] = ks[src];
            vd[idx] = vs[src];
        });
    }
    q.wait();
    snap_depth_ = depth;
    snap_tokens_.assign(tokens.begin(),
                        tokens.begin() + std::min<size_t>(tokens.size(), depth));
    return {};
}

uint32_t Gemma4Model::restore_kv(sycl::queue& q, uint32_t depth) {
    if (depth == 0 || snap_depth_ == 0 || kcache_.empty()) return 0;
    if (depth > snap_depth_) depth = snap_depth_;
    const uint32_t L = cfg_.n_layers, MC = kv_ctx_, SD = snap_depth_;
    // Scatter the first `depth` positions back to their live offsets (reverse of
    // the gather). The compact source stride is SD (the snapshot depth); positions
    // [0,depth) are the leading contiguous run within each kv-head's segment.
    for (uint32_t l = 0; l < L; ++l) {
        const uint32_t nkv = layers_[l].n_kv, hd = layers_[l].head_dim;
        const sycl::half* ks = snap_k_[l]; sycl::half* kd = kcache_[l];
        const sycl::half* vs = snap_v_[l]; sycl::half* vd = vcache_[l];
        const uint64_t total = uint64_t(nkv) * depth * hd;
        q.parallel_for(sycl::range<1>(total), [=](sycl::id<1> i) {
            const uint64_t idx = i[0];
            const uint32_t d   = uint32_t(idx % hd);
            const uint32_t pos = uint32_t((idx / hd) % depth);
            const uint32_t kv  = uint32_t(idx / (uint64_t(hd) * depth));
            const uint64_t src = (uint64_t(kv) * SD + pos) * hd + d;   // compact
            const uint64_t dst = (uint64_t(kv) * MC + pos) * hd + d;   // live
            kd[dst] = ks[src];
            vd[dst] = vs[src];
        });
    }
