    ...
    for (uint32_t dev = 0; dev < n_dev_; ++dev) {
        ...
        for (uint32_t L = 0; L < n_layers; ++L) {
            ...
            if (T == 1 && w.attn_partials) {
                ...
                // start_pos; else fp16 FA2. Mirrors qwen3moe.cpp / the crown split.
                if (kvc.is_int8() && kvc.k_int8_ptr() && start_pos == kvc.int8_length(li)) {
                    const uint64_t i8pl = uint64_t(d.n_kv_heads) * max_ctx * HD;
                    const uint64_t scpl = uint64_t(d.n_kv_heads) * max_ctx;
