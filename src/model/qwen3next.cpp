...
std::string Qwen3NextModel::load(DeviceFleet& fleet, const LayerPlan& plan,
    ...
    for (uint32_t dev = 0; dev < n_dev_; ++dev) {
        ...
        if (n_full[dev]) {
            ...
            kc.use_int8      = int8_kv;    // --int8-kv: int8 KV shadow on the full-attn layers only
...
std::string Qwen3NextModel::ensure_ws(uint32_t dev, uint32_t max_T) {
    ...
    w.moe_host_expert.resize(TK);
    w.moe_host_token.resize(TK);
    w.moe_host_identity.resize(TK);
    w.moe_host_weight.resize(TK);
    ...
    for (uint32_t dev = 0; dev < n_dev_; ++dev) {
        ...
        for (uint32_t L = 0; L < n_layers; ++L) {
            ...
            if (!lw.is_linear) {
                ...
                auto& kvc = kv_[dev];
                sycl::half* kc = kvc.k_ptr() + per_layer_kv * kv_local_[L];
                sycl::half* vc = kvc.v_ptr() + per_layer_kv * kv_local_[L];
                const uint32_t max_ctx = kvc.config().max_ctx;
                if (T == 1 && w.attn_partials) {
                    // INT8-KV decode (--int8-kv) when the int8 shadow is populated up
                    // to start_pos on this full-attn layer (card-local kv_local_[L]);
                    // else fp16 FA2. Mirrors the validated qwen3moe_split / qwen3moe_tp
                    // ports. No attention sinks. i8pl/scpl match per_layer_kv's layout.
                    if (kvc.is_int8() && kvc.k_int8_ptr() &&
                        start_pos == kvc.int8_length(kv_local_[L])) {
                        const uint64_t i8pl = uint64_t(n_kv) * max_ctx * HD;
                        const uint64_t scpl = uint64_t(n_kv) * max_ctx;
                        full_attention_fa2_decode_int8(
                            q, w.q, w.k, w.v,
                            kvc.k_int8_ptr()  + i8pl * kv_local_[L],
                            kvc.v_int8_ptr()  + i8pl * kv_local_[L],
                            kvc.k_scales_ptr() + scpl * kv_local_[L],
                            kvc.v_scales_ptr() + scpl * kv_local_[L],
                            nullptr, nullptr,
                            w.attn_out, w.attn_partials, start_pos,
                            n_q, n_kv, HD, max_ctx);
                        kvc.set_int8_length(kv_local_[L], start_pos + 1);
                    } else {
                ...
                kvc.set_length(kv_local_[L], start_pos + T);
                // Quantize prefill rows into the int8 shadow AFTER the whole prefill
                // branch → covers BOTH leaves (fa2_prefill_tile_gemma + naive
                // full_attention). quantize_to_int8 sets int8_lengths_[layer] so the
                // next decode takes the int8 path. Gated T>1 && is_int8.
                if (T > 1 && kvc.is_int8())
                    kvc.quantize_to_int8(q, kv_local_[L], start_pos, T);
            ...
            // combined FFN output lands in w.moe_y, then residual_add into w.x.
            {
                ...
                if (lw.gate_dt == DType::kEXL3) {
                    ...
                    static const bool async_route_upload =
                        std::getenv("IE_EXL3_MOE_ASYNC_UPLOAD") != nullptr;
                    if (!moe_slow) {
                        const uint32_t R = T * K;                 // active rows (token,expert)
                        auto& rexp = w.moe_host_expert;
                        auto& rtok = w.moe_host_token;
                        auto& ridn = w.moe_host_identity;
                        auto& rw   = w.moe_host_weight;
                        for (uint32_t t = 0; t < T; ++t)
                        ...
                        auto route_upload = q.memcpy(w.moe_wpk, rw.data(),
                                                     R * sizeof(sycl::half));
                        // Default keeps the validated behavior. The opt-in path is
                        // safe because host sources persist in Workspace and the next
                        // layer's router readback fences this in-order queue before reuse.
                        if (!async_route_upload) route_upload.wait();
