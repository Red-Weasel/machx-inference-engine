...
#include "ie/dequant_ref.hpp"    // ie::ref host dequant (universal fallback types)
...
T* upload(DeviceAllocator& alloc, const GgufTensorInfo* t,
    ...
    // Coerce BF16/F16 → F32 when an F32 tensor is expected. Community/Dreamified/
    // dynamic GGUFs often store the tiny norm/gate/router tensors in BF16 or F16;
    // converting here (host — these tensors are small) fixes the whole class of
    // "dtype mismatch: BF16 expected F32" load failures across every arch.
...
// IE_Q4K_SOA_V2 (DEFAULT ON): route the mainstream Q4_K decode GEMV (gemv_q_i8,
// attn q/k/v/o + ffn gate/up + ffn_down) through gemv_q4_K_soa_q8_v2 — the
// coalesced-load SoA int-dot kernel — instead of the AoS gemv_q4_K_q8. The loader
// repacks Q4_K weights to the SoA streams at upload (maybe_upload_q4k_soa_v2).
// Numerics == gemv_q4_K_q8 modulo cross-lane fp-reassociation (validated PPL-neutral
// on qwen3 + qwen2; +~13% dense decode). OPT-OUT with IE_Q4K_SOA_V2=0 (falls back to
// the byte-identical legacy AoS path). MoE/crown decode is unaffected (own kernels).
inline bool q4k_soa_v2_enabled() {
    // Default ON; only an explicit IE_Q4K_SOA_V2=0 disables it (unset / any other
    // value = ON).
    static const bool on = []{
        const char* e = std::getenv("IE_Q4K_SOA_V2");
        return !e || e[0] != '0';
    }();
    return on;
}

// When IE_Q4K_SOA_V2 is set, repack an already-uploaded AoS Q4_K weight into the
// five natural-order SoA streams (repack_q4_K_to_soa — bit-identical layout move,
// ie-q4soa-test) and stash the device pointers in `out.soa_q4_*` for
// gemv_q4_K_soa_q8_v2. ADDITIVE: `out.p` (AoS) is untouched (prefill / ffn_down /
// lm_head still use it). Best-effort — any skip (gate off, bad geometry, OOM)
// leaves soa_q4_q null so gemv_q_i8 transparently falls back to gemv_q4_K_q8.
inline void maybe_upload_q4k_soa_v2(DeviceAllocator& alloc,
                                    const GgufTensorInfo* t,
                                    DenseQuantPtr& out,
                                    std::vector<void*>& owned) {
    if (!q4k_soa_v2_enabled() || !t || t->dtype != DType::kQ4_K || t->n_dims != 2)
        return;
    const uint32_t K = uint32_t(t->shape[0]);   // ggml: shape[0] = contiguous (K)
    const uint32_t N = uint32_t(t->shape[1]);
    if (K == 0 || N == 0 || (K % kQK_K) != 0) return;   // AoS stays valid
    const uint64_t q_n  = uint64_t(N) * (K / 2);
    const uint64_t sc_n = uint64_t(N) * (K / 32);
    const uint64_t d_n  = uint64_t(N) * (K / 256);
    std::vector<uint8_t>  h_q(q_n, 0);
    std::vector<int8_t>   h_sc(sc_n, 0), h_mn(sc_n, 0);
    std::vector<uint16_t> h_d(d_n, 0), h_dm(d_n, 0);
    ...
    auto* d_mn = static_cast<int8_t*>(alloc.malloc(sc_n));
    auto* d_d  = static_cast<uint16_t*>(alloc.malloc(d_n * sizeof(uint16_t)));
    auto* d_dm = static_cast<uint16_t*>(alloc.malloc(d_n * sizeof(uint16_t)));
    if (!d_q || !d_sc || !d_mn || !d_d || !d_dm) return;   // OOM → AoS fallback
    ...
    alloc.queue().memcpy(d_mn, h_mn.data(), sc_n).wait();
    alloc.queue().memcpy(d_d,  h_d.data(),  d_n * sizeof(uint16_t)).wait();
    alloc.queue().memcpy(d_dm, h_dm.data(), d_n * sizeof(uint16_t)).wait();
    out.soa_q4_q = d_q; out.soa_q4_sc = d_sc; out.soa_q4_mn = d_mn;
    out.soa_q4_d = d_d; out.soa_q4_dmin = d_dm;
}

// IE_Q6K_SOA_V2 (DEFAULT ON): route the universal Q6_K lm_head decode GEMV
// through gemv_q6_soa_q8_v2 — the coalesced-load SoA int-dot kernel — instead of
// the AoS gemv_q6_K_q8. Every dense GGUF's lm_head (output / tied token_embd) is
// Q6_K, so a coalesced Q6 lm_head helps EVERY model's decode. Numerics ==
// gemv_q6_K_q8 modulo cross-lane fp-reassociation (both W6A8 int-dot; pure layout
// move). OPT-OUT with IE_Q6K_SOA_V2=0 (falls back to the AoS int-dot path).
inline bool q6k_soa_v2_enabled() {
    static const bool on = []{
        const char* e = std::getenv("IE_Q6K_SOA_V2");
        return !e || e[0] != '0';   // unset / any non-'0' = ON; opt out with =0
    }();
    return on;
}

// Repack an AoS Q6_K weight into the four natural-order SoA-Q6 streams
// (repack_q6_K_to_soa — bit-identical layout move) and stash the device pointers
// in `out.soa_q6_*` for gemv_q6_soa_q8_v2. ADDITIVE: `out.p` (AoS) is untouched.
// Best-effort — any skip (gate off, non-Q6_K, bad geometry, OOM) leaves
// soa_q6_lo null so the lm_head decode transparently falls back to gemv_q6_K_q8.
// NOTE: h_lo/h_hi MUST be zero-init (the bit-plane repack is read-modify-write).
inline void maybe_upload_q6k_soa_v2(DeviceAllocator& alloc,
                                    const GgufTensorInfo* t,
                                    DenseQuantPtr& out,
                                    std::vector<void*>& owned) {
    if (!q6k_soa_v2_enabled() || !t || t->dtype != DType::kQ6_K || t->n_dims != 2)
        return;
    const uint32_t K = uint32_t(t->shape[0]);   // ggml: shape[0] = contiguous (K)
    const uint32_t N = uint32_t(t->shape[1]);
    if (K == 0 || N == 0 || (K % kQK_K) != 0) return;   // AoS stays valid
    ...
    auto* d_sc = static_cast<int8_t*>(alloc.malloc(sc_n));
    auto* d_d  = static_cast<uint16_t*>(alloc.malloc(d_n * sizeof(uint16_t)));
    if (!d_lo || !d_hi || !d_sc || !d_d) return;   // OOM → AoS fallback
    ...
    alloc.queue().memcpy(d_sc, h_sc.data(), sc_n).wait();
    alloc.queue().memcpy(d_d,  h_d.data(),  d_n * sizeof(uint16_t)).wait();
    out.soa_q6_lo = d_lo; out.soa_q6_hi = d_hi;
    out.soa_q6_sc = d_sc; out.soa_q6_d  = d_d;
}
...
inline DenseQuantPtr upload_quant_dense(DeviceAllocator& alloc,
    ...
    out.p = d;
    out.dt = t->dtype;
    maybe_upload_q4k_soa_v2(alloc, t, out, owned);   // IE_Q4K_SOA_V2 opt-in (no-op otherwise)
    return out;
}
...
// True iff `dt` rides the host universal dequant→fp16 fallback (a valid GGUF
// weight quant with no native/device fast path yet — see upload_host_dequant_to_fp16).
inline bool is_host_fallback_dtype(DType dt) noexcept {
    return dt == DType::kQ4_0 || dt == DType::kQ4_1 ||
           dt == DType::kQ5_0 || dt == DType::kQ5_1 ||
           dt == DType::kQ2_K || dt == DType::kQ3_K ||
           dt == DType::kIQ4_NL || dt == DType::kIQ4_XS ||
           dt == DType::kIQ3_XXS;
}

// Universal load fallback: host-dequant a 2-D weight of a quant type that has NO
// device _to_Bt kernel (Q4_1/Q5_0/Q5_1/Q2_K/Q3_K) to device F16 [K,N], the same
// transposed layout the device dequant path produces, so the downstream fp16
// GEMV/GEMM consumes it identically. Correctness-first (llama.cpp's guarantee: a
// missing fast kernel makes a quant SLOWER, never unloadable). ie::ref is the
// bit-exact-vs-ggml host reference (tests/unit/dequant_ref_test). Slower than a
// device kernel (host scalar loop, once at load) — a perf follow-up may add
// dequant_*_to_Bt kernels, but this is never a load gate.
inline DenseQuantPtr upload_host_dequant_to_fp16(DeviceAllocator& alloc,
                                                 const GgufTensorInfo* t,
                                                 std::vector<void*>& owned,
                                                 std::string& err) {
    DenseQuantPtr out;
    if (!t) { err = "tensor not found"; return out; }
    if (t->n_dims != 2) { err = "host_dequant: expected 2-D weight"; return out; }
    const uint64_t K = t->shape[0], N = t->shape[1];
    if (K == 0 || N == 0) { err = "host_dequant: zero dim"; return out; }
    const uint64_t n = K * N;
    // ggml-natural order: f32[nn*K + k] (nn output rows of K contiguous elems).
    std::vector<float> f32(n);
    switch (t->dtype) {
        case DType::kQ4_0: ie::ref::dequant_q4_0_buffer(t->data, n, f32.data()); break;
        case DType::kQ4_1: ie::ref::dequant_q4_1_buffer(t->data, n, f32.data()); break;
        case DType::kQ5_0: ie::ref::dequant_q5_0_buffer(t->data, n, f32.data()); break;
        case DType::kQ5_1: ie::ref::dequant_q5_1_buffer(t->data, n, f32.data()); break;
        case DType::kQ2_K: ie::ref::dequant_q2_K_buffer(t->data, n, f32.data()); break;
        case DType::kQ3_K: ie::ref::dequant_q3_K_buffer(t->data, n, f32.data()); break;
        case DType::kIQ4_NL: ie::ref::dequant_iq4_nl_buffer(t->data, n, f32.data()); break;
        case DType::kIQ4_XS: ie::ref::dequant_iq4_xs_buffer(t->data, n, f32.data()); break;
        case DType::kIQ3_XXS: ie::ref::dequant_iq3_xxs_buffer(t->data, n, f32.data()); break;
        default:
            err = std::string("host_dequant: unsupported ") + std::string(type_name(t->dtype));
            return out;
    }
    // Transpose [N,K] → [K,N] fp16 (staging[k*N + nn]) — identical to the F16 and
    // device-dequant upload layouts.
    std::vector<uint16_t> staging(n);
    for (uint64_t nn = 0; nn < N; ++nn)
        for (uint64_t k = 0; k < K; ++k)
            staging[k * N + nn] = fp32_to_fp16(f32[nn * K + k]);
    void* d = alloc.malloc(n * sizeof(uint16_t));
    if (!d) { err = "malloc failed (host_dequant fp16)"; return out; }
    alloc.queue().memcpy(d, staging.data(), n * sizeof(uint16_t)).wait();
    owned.push_back(d);
    out.p = d;
    out.dt = DType::kF16;
    return out;
...
// Auto upload: Q5_K/Q8_0 → device fp16 dequant; Q4_1/Q5_0/Q5_1/Q2_K/Q3_K → host
// fp16 dequant fallback; Q4_K/Q6_K/F16/BF16 ride upload_quant_dense. Sibling of
// upload_quant_dense (NOT a widening of it) — opt-in only, so the crown/validated
// dense loads that keep calling upload_quant_dense are byte-identical.
inline DenseQuantPtr upload_quant_dense_auto(DeviceAllocator& alloc,
    ...
    if (is_host_fallback_dtype(t->dtype))
        return upload_host_dequant_to_fp16(alloc, t, owned, err);
    return upload_quant_dense(alloc, t, owned, err);
}

// Universal token-embedding fallback. Dequant an embedding TABLE of any quant
// type that lacks a native embedding_lookup kernel → F16 in ggml-NATURAL order
// ([vocab][hidden], hidden contiguous — NO transpose), so embedding_lookup_f16
// consumes it directly. token_embd ships [hidden,vocab] in the GGUF header but is
// vocab-row-major (hidden contiguous), so a flat natural dequant already yields
// the [vocab][hidden] F16 table (same as the crown APEX BF16 path, c965936).
// Correctness-first: a missing fast kernel makes a quant load SLOWER (host scalar
// loop, once) but NEVER unloadable — llama.cpp's universality guarantee. Returns
// a raw F16 device pointer (caller sets token_embd_dtype_ = kF16).
inline void* upload_embd_any_to_f16(DeviceAllocator& alloc,
                                    const GgufTensorInfo* t,
                                    std::vector<void*>& owned,
                                    std::string& err) {
    if (!t) { err = "tensor not found"; return nullptr; }
    if (t->n_dims != 2) { err = "embd_dequant: expected 2-D token_embd"; return nullptr; }
    const uint64_t n = uint64_t(t->shape[0]) * uint64_t(t->shape[1]);
    if (n == 0) { err = "embd_dequant: zero dim"; return nullptr; }
    ...
    if (t->dtype == DType::kF16) {
        ...
        const auto* src = reinterpret_cast<const uint16_t*>(t->data);
        for (uint64_t i = 0; i < n; ++i) {
            uint32_t bits = uint32_t(src[i]) << 16; float f; std::memcpy(&f, &bits, sizeof(f));
            f16[i] = fp32_to_fp16(f);
        }
    } else if (t->dtype == DType::kF32) {
        const auto* src = reinterpret_cast<const float*>(t->data);
        for (uint64_t i = 0; i < n; ++i) f16[i] = fp32_to_fp16(src[i]);
    } else {
        std::vector<float> f32(n);
        switch (t->dtype) {
            case DType::kQ4_0: ie::ref::dequant_q4_0_buffer(t->data, n, f32.data()); break;
            ...
            case DType::kQ5_0: ie::ref::dequant_q5_0_buffer(t->data, n, f32.data()); break;
            case DType::kQ5_1: ie::ref::dequant_q5_1_buffer(t->data, n, f32.data()); break;
            case DType::kQ8_0: ie::ref::dequant_q8_0_buffer(t->data, n, f32.data()); break;
            case DType::kQ2_K: ie::ref::dequant_q2_K_buffer(t->data, n, f32.data()); break;
            case DType::kQ3_K: ie::ref::dequant_q3_K_buffer(t->data, n, f32.data()); break;
            case DType::kQ4_K: ie::ref::dequant_q4_K_buffer(t->data, n, f32.data()); break;
            case DType::kQ5_K: ie::ref::dequant_q5_K_buffer(t->data, n, f32.data()); break;
            case DType::kQ6_K: ie::ref::dequant_q6_K_buffer(t->data, n, f32.data()); break;
            case DType::kIQ4_NL: ie::ref::dequant_iq4_nl_buffer(t->data, n, f32.data()); break;
            case DType::kIQ4_XS: ie::ref::dequant_iq4_xs_buffer(t->data, n, f32.data()); break;
            case DType::kIQ3_XXS: ie::ref::dequant_iq3_xxs_buffer(t->data, n, f32.data()); break;
            default:
                err = std::string("token_embd: no dequant for ") + std::string(type_name(t->dtype));
                return nullptr;
        }
        for (uint64_t i = 0; i < n; ++i) f16[i] = fp32_to_fp16(f32[i]);
    }
    void* d = alloc.malloc(n * sizeof(uint16_t));
    if (!d) { err = "malloc failed (embd f16)"; return nullptr; }
    alloc.queue().memcpy(d, f16.data(), n * sizeof(uint16_t)).wait();
    owned.push_back(d);
    return d;
}
...
inline DenseQuantPtr upload_quant_dense_row_slice_permuted(
    ...
    // Q5_K/Q8_0 (device dequant) and the host-fallback quants (Q4_1/Q5_0/Q5_1/
    // Q2_K/Q3_K): byte-shuffle the superblock-rows in a host staging buffer (dest
    // row i ← src row perm[i]), then dequant-to-fp16 the un-permuted superblocks
    // via upload_quant_dense_auto. Each output row is a contiguous whole-superblock
    // span (row_bytes = nbytes/N), so the row-shuffle is a pure byte move for ANY
    // block quant — no requant, no dtype-specific unpacking here.
    if (s.dtype == DType::kQ5_K || s.dtype == DType::kQ8_0 ||
        is_host_fallback_dtype(s.dtype)) {
...
inline sycl::event gemv_q_i8(sycl::queue& q, const sycl::half* x,
    ...
    if (x_q8 && W.dt == DType::kQ4_K) {
        // IE_Q4K_SOA_V2: coalesced-load SoA int-dot decode kernel (numerics ==
        // gemv_q4_K_q8). Only when the loader repacked this weight; else AoS.
        if (q4k_soa_v2_enabled() && W.soa_q4_q)
            return gemv_q4_K_soa_q8_v2(
                q, x_q8,
                static_cast<const uint8_t*>(W.soa_q4_q),
                static_cast<const int8_t*>(W.soa_q4_sc),
                static_cast<const int8_t*>(W.soa_q4_mn),
                static_cast<const uint16_t*>(W.soa_q4_d),
                static_cast<const uint16_t*>(W.soa_q4_dmin),
                y, K, N);
...
inline sycl::event gemv_q_T(sycl::queue& q, const sycl::half* A,
    ...
    if (W.dt == DType::kEXL3) {
        // Decode each ALU-bound trellis tile once and MAC it across the prefill
        // batch. Keep the original per-row path as a diagnostic rollback.
        static const bool batched =
            std::getenv("IE_EXL3_NO_BATCHED_PREFILL") == nullptr;
        if (batched)
            return gemm_exl3(q, A, W.p,
                             static_cast<const sycl::half*>(W.suh),
                             static_cast<const sycl::half*>(W.svh),
                             y, T, K, N, W.bits, deps);
        // else fall through to the per-token loop below (unchanged path).
    }
