...
T* upload(DeviceAllocator& alloc, const GgufTensorInfo* t,
    ...
    // in community/Dreamified/dynamic GGUFs). Small tensors → cheap host convert.
...
// IE_Q4K_SOA_V2 (DEFAULT ON): route the crown's local Q4_K decode GEMV (gemv_q_i8 —
// attn q/k/v/o + DeltaNet qkv/gate) through gemv_q4_K_soa_q8_v2, the coalesced-load
// SoA int-dot kernel that ships default-ON on the dense path (dense_dispatch.hpp).
// Mirrors dense::q4k_soa_v2_enabled — same env var, one choke-point read by both the
// load-time repack (maybe_upload_q4k_soa_v2) and the decode dispatch. Numerics ==
// gemv_q4_K_q8 modulo cross-lane fp-reassociation (validated PPL-neutral on dense
// qwen3/qwen2). OPT-OUT with IE_Q4K_SOA_V2=0 → byte-identical legacy AoS path. The
// crown's 3-D MoE expert banks are untouched (own SoA path; n_dims==2 guard below).
inline bool q4k_soa_v2_enabled() {
    static const bool on = []{
        const char* e = std::getenv("IE_Q4K_SOA_V2");
        return !e || e[0] != '0';   // unset / any non-'0' value = ON
    }();
    return on;
}
...
// When IE_Q4K_SOA_V2 is on, repack an already-uploaded AoS Q4_K weight into the five
// natural-order SoA streams (repack_q4_K_to_soa — bit-identical layout move, proven by
// ie-q4soa-test) and stash the device pointers in `out.soa_q4_*` for gemv_q4_K_soa_q8_v2.
// Faithful mirror of dense::maybe_upload_q4k_soa_v2 (dense_dispatch.hpp) writing into the
// crown's QuantPtr instead of DenseQuantPtr. ADDITIVE: `out.p` (AoS blocks) is untouched
// (prefill gemv_q_T still uses it). The n_dims==2 guard keeps the 3-D MoE expert banks on
// the raw/per-expert-SoA path (they are loaded via upload_quant_soa, never here, but this
// fails safe regardless). Best-effort — any skip (gate off, non-Q4_K, bad geometry, OOM)
// leaves soa_q4_q null so gemv_q_i8 transparently falls back to gemv_q4_K_q8.
void maybe_upload_q4k_soa_v2(DeviceAllocator& alloc, const GgufTensorInfo* t,
                             QuantPtr& out, std::vector<void*>& owned) {
...
// IE_Q6K_SOA (DEFAULT ON): route the crown's Q6_K decode GEMVs (the "med" bucket —
// attn_qkv / attn_v Q6_K via gemv_q_i8 — and the "huge" lm_head) through
// gemv_q6_soa_q8, the SoA int-dot W6A8 kernel proven on the 27B/dense Q6 fast path
// (took 27B ffn_down 5.17→12.9 tok/s). Same env-choke-point pattern as
// q4k_soa_v2_enabled. Numerics == the AoS gemv_q6_K_q8 / gemv_q6_K modulo int8-act
// quant + fp-fold order — measured crown Q4_K_M PPL 5.4147→5.4197 (+0.09%, gate ≤6.57
// held) since the attn_qkv/attn_v projections move W6A16→W6A8 (consistent with the
// crown's already-default int8-act attention for its Q4_K projections). Opt OUT with
// IE_Q6K_SOA=0 → byte-identical legacy AoS/float path. The 3-D MoE expert banks are
// untouched (own per-expert SoA path; n_dims==2 guard below).
inline bool q6k_soa_enabled() {
    static const bool on = []{
        const char* e = std::getenv("IE_Q6K_SOA");
        return !e || e[0] != '0';   // unset / any non-'0' = ON; opt out with =0
    }();
    return on;
}

// IE_Q6K_SOA_V2 (DEFAULT ON): among the SoA-Q6 path (IE_Q6K_SOA), select the
// COALESCED-load kernel gemv_q6_soa_q8_v2 over the split-K v1 gemv_q6_soa_q8.
// Both consume the SAME repacked SoA-Q6 streams (no reload) — v2 remaps the 16
// lanes so each owns one per-16 scale-group of the same super-block (all 16 lanes
// active + coalesced hi/scale loads), vs v1's whole-super-block per-lane scatter
// and 8/16-lane use at K=2048. Numerics == v1 (last-bit fp reassoc). Opt OUT with
// IE_Q6K_SOA_V2=0 → the split-K v1 kernel. Lets us A/B v2 vs v1 on one binary.
...
// When IE_Q6K_SOA is on, repack an already-uploaded AoS Q6_K weight into the four
// natural-order SoA-Q6 streams (repack_q6_K_to_soa — bit-identical layout move) and
// stash the device pointers in `out.soa_q6_*` for gemv_q6_soa_q8. Faithful mirror of
// maybe_upload_q4k_soa_v2 above (and of qwen35_dense::upload_q6_soa). ADDITIVE:
// `out.p` (AoS blocks) is untouched (prefill gemv_q_T still uses it). The n_dims==2
// guard keeps the 3-D MoE expert banks on the raw/per-expert-SoA path. Best-effort —
// any skip (gate off, non-Q6_K, bad geometry, OOM) leaves soa_q6_lo null so the
// decode dispatch transparently falls back to the AoS gemv_q6_K_q8 / gemv_q6_K.
void maybe_upload_q6k_soa(DeviceAllocator& alloc, const GgufTensorInfo* t,
                          QuantPtr& out, std::vector<void*>& owned) {
    if (!q6k_soa_enabled() || !t || t->dtype != DType::kQ6_K || t->n_dims != 2)
        return;
    const uint32_t K = uint32_t(t->shape[0]);   // ggml: shape[0] = contiguous (K)
    const uint32_t N = uint32_t(t->shape[1]);
    if (K == 0 || N == 0 || (K % kQK_K) != 0) return;   // AoS stays valid
    ...
    repack_q6_K_to_soa(t->data, K, N, h_lo.data(), h_hi.data(), h_sc.data(),
                       h_d.data());
...
sycl::event gemv_q_i8(sycl::queue& q, const sycl::half* x, const void* x_q8,
    ...
    if (x_q8 && W.dt == DType::kQ4_K) {
        // IE_Q4K_SOA_V2 (default ON): coalesced-load SoA int-dot decode kernel when
        // the loader repacked this weight (maybe_upload_q4k_soa_v2). Transparent AoS
        // fallback (gemv_q4_K_q8) when the gate is off or the repack was skipped.
        if (q4k_soa_v2_enabled() && W.soa_q4_q)
            return gemv_q4_K_soa_q8_v2(q, x_q8,
    ...
    if (x_q8 && W.dt == DType::kQ6_K) {
        // IE_Q6K_SOA (default OFF): coalesced SoA int-dot decode for Q6_K attn
        // projections (attn_qkv / attn_v) when the loader repacked this weight
        // (maybe_upload_q6k_soa). Transparent fallback to the FLOAT gemv_q6_K (via
        // gemv_q below) when the gate is off or the repack was skipped.
        if (q6k_soa_enabled() && W.soa_q6_lo) {
            // IE_Q6K_SOA_V2 (default ON): coalesced-load kernel; opt-out → v1 split-K.
            if (q6k_soa_v2_enabled())
                return gemv_q6_soa_q8_v2(q, x_q8,
                    static_cast<const uint8_t*>(W.soa_q6_lo),
                    static_cast<const uint8_t*>(W.soa_q6_hi),
                    static_cast<const int8_t*>(W.soa_q6_sc),
                    static_cast<const uint16_t*>(W.soa_q6_d),
                    y, K, N);
            return gemv_q6_soa_q8(q, x_q8,
                static_cast<const uint8_t*>(W.soa_q6_lo),
                static_cast<const uint8_t*>(W.soa_q6_hi),
                static_cast<const int8_t*>(W.soa_q6_sc),
                static_cast<const uint16_t*>(W.soa_q6_d),
...
std::string QwenModel::load(DeviceAllocator& alloc, const GgufReader& g, const QwenConfig& cfg) {
    ...
    if (!err.empty()) return "output_norm: " + err;
    {
        ...
        // IE_Q6K_SOA: repack the (Q6_K) lm_head to SoA-Q6 streams for the coalesced
        // int-dot decode (gemv_q6_soa_q8). No-op unless Q6_K + gate on; leaves
        // output_q6soa_.soa_q6_lo null → AoS gemv_q6_K_q8 fallback at the lm_head site.
        maybe_upload_q6k_soa(alloc, ti, output_q6soa_, owned_);
    }
    ...
    for (uint32_t L = 0; L < cfg.n_layers; ++L) {
        ...
        // IE_Q4K_SOA_V2 loader: upload_quant + repack this Q4_K weight to the
        // coalesced-load SoA streams (gemv_q4_K_soa_q8_v2 decode). Used ONLY for the
        // 2-D dense Q4_K projections that decode via gemv_q_i8 (attn q/k/v/o +
        // DeltaNet qkv/gate) — not the shexp/ssm projections (dual/fused kernels,
        // never gemv_q_i8) nor the 3-D MoE banks. Reuses `t` so Tlayer's shared buf
        // is snprintf'd once. Repack is a no-op when the gate is off or dtype != Q4_K.
        auto LQv2 = [&](const char* name) -> QuantPtr {
            const auto* t = Tlayer(L, name);
            QuantPtr qp = upload_quant(alloc, t, owned_, err);
            if (err.empty()) {
                maybe_upload_q4k_soa_v2(alloc, t, qp, owned_);   // no-op unless Q4_K
                maybe_upload_q6k_soa(alloc, t, qp, owned_);      // no-op unless Q6_K
            }
            return qp;
        };
        ...
        if (full_attn) {
            w.attn_q       = LQv2("attn_q.weight");    if (!err.empty()) return std::string("layer ") + std::to_string(L) + " attn_q: " + err;
            w.attn_k       = LQv2("attn_k.weight");    if (!err.empty()) return std::string("layer ") + std::to_string(L) + " attn_k: " + err;
            w.attn_v       = LQv2("attn_v.weight");    if (!err.empty()) return std::string("layer ") + std::to_string(L) + " attn_v: " + err;
            w.attn_output  = LQv2("attn_output.weight"); if (!err.empty()) return std::string("layer ") + std::to_string(L) + " attn_output: " + err;
            ...
            w.attn_qkv     = LQv2("attn_qkv.weight");   if (!err.empty()) return std::string("layer ") + std::to_string(L) + " attn_qkv: " + err;
            w.attn_gate    = LQv2("attn_gate.weight");  if (!err.empty()) return std::string("layer ") + std::to_string(L) + " attn_gate: " + err;
    ...
    auto run_lmhead = [&]() -> sycl::event {
        ...
        if (T == 1 && q8_decode_enabled() && ws_q8_ && cfg_.vocab >= 32768) {
            if (lm_head.dt == DType::kQ6_K) {
                // IE_Q6K_SOA (default OFF): coalesced SoA int-dot lm_head decode.
                // Transparent AoS gemv_q6_K_q8 fallback when off / repack skipped.
                if (q6k_soa_enabled() && output_q6soa_.soa_q6_lo) {
                    // IE_Q6K_SOA_V2 (default ON): coalesced-load lm_head decode.
                    if (q6k_soa_v2_enabled())
                        return gemv_q6_soa_q8_v2(q, ws_q8_,
                            static_cast<const uint8_t*>(output_q6soa_.soa_q6_lo),
                            static_cast<const uint8_t*>(output_q6soa_.soa_q6_hi),
                            static_cast<const int8_t*>(output_q6soa_.soa_q6_sc),
                            static_cast<const uint16_t*>(output_q6soa_.soa_q6_d),
                            out_logits, H, cfg_.vocab);
                    return gemv_q6_soa_q8(q, ws_q8_,
                        static_cast<const uint8_t*>(output_q6soa_.soa_q6_lo),
                        static_cast<const uint8_t*>(output_q6soa_.soa_q6_hi),
                        static_cast<const int8_t*>(output_q6soa_.soa_q6_sc),
                        static_cast<const uint16_t*>(output_q6soa_.soa_q6_d),
