...
std::string Qwen3MoeTpModel::load(DeviceFleet& fleet, const GgufReader& g,
    ...
    for (uint32_t c = 0; c < n_dev_; ++c) {
        ...
        kc.max_ctx       = max_ctx;
        kc.head_dim      = HD;
        kc.use_int8      = int8_kv;          // --int8-kv: halve per-card KV (int8 shadow + fp16 scales)
...
std::string Qwen3MoeTpModel::forward(const int32_t* input_ids, uint32_t T,
    ...
    for (uint32_t L = 0; L < n_layers; ++L) {
        ...
        for (uint32_t c = 0; c < n_dev_; ++c) {
            ...
            auto& kvc = kv_[c];
            const uint64_t per_layer = uint64_t(snkv) * kvc.config().max_ctx * HD;
            sycl::half* kc = kvc.k_ptr() + per_layer * L;
            sycl::half* vc = kvc.v_ptr() + per_layer * L;
            const uint32_t mc = kvc.config().max_ctx;
            if (T == 1 && fa2_decode_on && ws.attn_partials) {
                // INT8-KV decode (--int8-kv) when the int8 shadow is populated up to
                // start_pos; else fp16 FA2. Mirrors qwen3moe_split.cpp (per-card, layer L,
                // head-sharded snkv kv-heads/card). NOT bit-exact vs single-GPU (reduction
                // order) — validated via coherent gen + untouched single-GPU PPL gate.
                if (kvc.is_int8() && kvc.k_int8_ptr() && start_pos == kvc.int8_length(L)) {
                    const uint64_t i8pl = uint64_t(snkv) * mc * HD;
                    const uint64_t scpl = uint64_t(snkv) * mc;
                    full_attention_fa2_decode_int8(q, ws.q, ws.k, ws.v,
                                                   kvc.k_int8_ptr() + i8pl * L,
                                                   kvc.v_int8_ptr() + i8pl * L,
                                                   kvc.k_scales_ptr() + scpl * L,
                                                   kvc.v_scales_ptr() + scpl * L,
                                                   nullptr, nullptr,
                                                   ws.attn_out, ws.attn_partials, start_pos,
                                                   snq, snkv, HD, mc);
                    kvc.set_int8_length(L, start_pos + 1);
                } else {
            ...
            kvc.set_length(L, start_pos + T);
            // Quantize prefill rows into the int8 shadow AFTER the branch → covers BOTH
            // prefill leaves (fa2_prefill_tile and naive full_attention). quantize_to_int8
            // sets int8_lengths_[L] internally so the next decode takes the int8 path.
            if (T > 1 && kvc.is_int8()) kvc.quantize_to_int8(q, L, start_pos, T);
