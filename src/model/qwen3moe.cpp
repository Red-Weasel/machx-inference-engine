...
std::string Qwen3MoeModel::ensure_workspace(uint32_t max_T) {
    ...
    ws_eout_     = ah(H);
    ws_acc_      = ah(H);
    // block_q8_1x activation scratch for the coalesced Q6_K lm_head decode (v2).
    ws_lm_q8_ = alloc_->malloc((H / 32) * sizeof(block_q8_1x));
    if (ws_lm_q8_) owned_.push_back(ws_lm_q8_);
    ...
    if (!ws_x_ || !ws_xn_ || !ws_q_ || !ws_k_ || !ws_v_ || !ws_attn_out_ || !ws_block_ ||
        ...
        !ws_router_logits_ || !ws_xp_q8_ || !ws_h_q8_ || !ws_lm_q8_)
...
std::string Qwen3MoeModel::load(DeviceAllocator& alloc, const GgufReader& g,
    ...
    // Q6 decode kernel), mirroring the dense/crown lm_head wiring. Coder-30B's
    // lm_head is Q6_K (separate output.weight OR the tied token_embd), so this
    // routes its decode off the FLOAT gemv_q6k_huge. No-op unless Q6_K + n_dims==2;
    // transparent FLOAT gemv_q fallback on any skip (gate off / bad geom / OOM).
...
sycl::event Qwen3MoeModel::forward(sycl::queue& q, const int32_t* input_ids, uint32_t T,
    ...
    // (W6A8) — mirrors the dense/crown lm_head win, off the FLOAT gemv_q6k_huge.
    // Only the last token's logits are computed, so this single-row GEMV needs a
    // one-row Q8_1 quant of `last`. Gate on T==1 (decode) + Q6_K + big vocab +
    // repacked streams; float gemv_q fallback otherwise (incl. prefill).
    if (T == 1 && output_dtype_ == DType::kQ6_K && d.vocab >= 32768 &&
        dense::q6k_soa_v2_enabled() && output_soa_q6_lo_ && ws_lm_q8_) {
        quantize_q8_1(q, last, ws_lm_q8_, H);
        return gemv_q6_soa_q8_v2(q, ws_lm_q8_,
