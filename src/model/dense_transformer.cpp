...
std::string DenseModel::load(DeviceAllocator& alloc, const GgufReader& g,
    ...
    // Top-level — token_embd rides its native embedding_lookup for Q4_K/Q6_K/F16;
    // ANY other quant (Q8_0, Q5_K, Q4_0/1, Q5_0/1, Q2_K, Q3_K, IQ4*, BF16, F32) is
    // host-dequanted to an F16 table so a stranger's GGUF never fails to load on an
    // embedding dtype (universal fallback, upload_embd_any_to_f16).
    {
        ...
        if (ti->dtype == DType::kQ4_K || ti->dtype == DType::kQ6_K ||
            ...
            token_embd_ = dense::upload_embd_any_to_f16(alloc, ti, owned_, err);
            if (!err.empty()) return "token_embd: " + err;
    ...
    {
        // 0.6B–4B Qwen3-dense GGUFs tie embeddings: output.weight is absent and the
        // lm_head IS token_embd. For Q4_K/Q6_K the embedding table is stored in the
        // SAME natural layout the logits GEMV reads, so reuse it (no extra VRAM). A
        // dequanted / native-F16 embedding table is in embedding-natural order,
        // which is NOT the F16 GEMV layout, so re-load a proper lm_head from
        // token_embd via the self-consistent auto path (transposes as the GEMV wants).
        const auto* ti = T("output.weight");
        if (!ti) {
            if (token_embd_dtype_ == DType::kQ4_K || token_embd_dtype_ == DType::kQ6_K) {
                output_ = token_embd_;
                output_dtype_ = token_embd_dtype_;
            } else {
                DenseQuantPtr o = dense::upload_quant_dense_auto(
                        alloc, T("token_embd.weight"), owned_, err);
                if (!err.empty()) return "lm_head (tied): " + err;
                output_ = o.p; output_dtype_ = o.dt;
            }
            ...
            // Universal: Q4_K/Q6_K ride their native GEMV; any other quant is
            // dequanted to F16 (self-consistent auto path → matching GEMV layout).
            DenseQuantPtr o = dense::upload_quant_dense_auto(alloc, ti, owned_, err);
            if (!err.empty()) return "output: " + err;
            output_ = o.p; output_dtype_ = o.dt;
        }
    }

    // Universal Q6_K lm_head coalesced-decode (IE_Q6K_SOA_V2, default ON): repack
    // the lm_head weight into SoA-Q6 streams for gemv_q6_soa_q8_v2 (the coalesced
    // Q6 decode kernel). Every dense GGUF's lm_head is Q6_K (separate output.weight
    // OR the tied token_embd), so this helps EVERY model's decode. No-op unless the
    // lm_head is Q6_K + n_dims==2; transparent AoS gemv_q6_K_q8 fallback on any skip.
    if (output_dtype_ == DType::kQ6_K) {
        const GgufTensorInfo* lmw = T("output.weight");
        if (!lmw) lmw = T("token_embd.weight");   // tied lm_head
        DenseQuantPtr tmp;
        dense::maybe_upload_q6k_soa_v2(alloc, lmw, tmp, owned_);
        output_soa_q6_lo_ = tmp.soa_q6_lo; output_soa_q6_hi_ = tmp.soa_q6_hi;
        output_soa_q6_sc_ = tmp.soa_q6_sc; output_soa_q6_d_  = tmp.soa_q6_d;
    }
...
sycl::event DenseModel::forward(sycl::queue& q,
    ...
    for (uint32_t L = 0; L < cfg_.n_layers; ++L) {
        ...
        if (down_q8_0_repack) {
            ...
            if (w.ffn_down.dt == DType::kQ4_K) {
                // Lever A (IE_Q4K_SOA_V2): route the big-K ffn_down Q4_K decode
                // through the coalesced-load SoA kernel too. ffn_down calls this
                // DIRECTLY (not via dense::gemv_q_i8), so it was the residual
                // gemv_q4_K_q8 chunk after Stage 1. The loader already repacked
                // its SoA streams (maybe_upload_q4k_soa_v2 fires for EVERY Q4_K
                // dense tensor), so this is a pure routing move — numerics ==
                // gemv_q4_K_q8. Transparent AoS fallback on any repack skip
                // (gate off / bad geometry / OOM → soa_q4_q stays null).
                if (dense::q4k_soa_v2_enabled() && w.ffn_down.soa_q4_q)
                    gemv_q4_K_soa_q8_v2(
                        q, ws_q8_,
                        static_cast<const uint8_t*>(w.ffn_down.soa_q4_q),
                        static_cast<const int8_t*>(w.ffn_down.soa_q4_sc),
                        static_cast<const int8_t*>(w.ffn_down.soa_q4_mn),
                        static_cast<const uint16_t*>(w.ffn_down.soa_q4_d),
                        static_cast<const uint16_t*>(w.ffn_down.soa_q4_dmin),
                        ws_attn_block_, F, H);
                else
    ...
    sycl::event head_ev;
    if (head_q8 && cfg_.vocab >= 32768 && lm_head.dt == DType::kQ6_K &&
        dense::q6k_soa_v2_enabled() && output_soa_q6_lo_)
        // IE_Q6K_SOA_V2 (default ON): coalesced-load SoA int-dot lm_head decode.
        head_ev = gemv_q6_soa_q8_v2(q, ws_q8_,
            static_cast<const uint8_t*>(output_soa_q6_lo_),
            static_cast<const uint8_t*>(output_soa_q6_hi_),
            static_cast<const int8_t*>(output_soa_q6_sc_),
            static_cast<const uint16_t*>(output_soa_q6_d_),
            out_logits, H, cfg_.vocab);
    else if (head_q8 && cfg_.vocab >= 32768 &&
