...
std::string import_exl3_to_gguf(const std::string& hf_dir, const std::string& out_gguf,
    ...
    // Dense EXL3 covers BOTH Llama and Qwen3-dense. The tensor-name map is
    // family-generic (q/k/v/o + gate/up/down + RMSNorms), and Qwen3's only
    // structural delta — per-head q_norm/k_norm — rides the 1-D norm branch
    // (→ attn_q_norm/attn_k_norm) and is applied by the dense forward on tensor
    // PRESENCE, not arch. RoPE stays correct because EXL3 codes are HF-natural
    // NEOX order (never row-un-permuted; the kLlama3 un-permute is is_exl3-guarded
    // off), so emitting general.architecture="qwen3" routes to kQwen3Dense cleanly.
    if (meta.arch != "llama" && meta.arch != "qwen3")
        return "exl3 dense import supports model_type 'llama' or 'qwen3' (got '" +
               meta.arch + "')";
    note("config: " + meta.arch + " dense, " + std::to_string(meta.cfg.n_layers) +
         " layers, hidden " + std::to_string(meta.cfg.hidden) + ", exl3");
    ...
    GgufWriter w;
    const std::string A = meta.arch;   // "llama" or "qwen3" — self-identifying GGUF arch
    w.kv_string("general.architecture", A);
...
// contract is the bartowski Q4_K_M qwen3next (diff target).
// ===================================================================== //
...
std::string import_hf_to_gguf(const std::string& hf_dir,
    ...
    HfModelMeta meta;
    if (auto e = parse_hf_config(cfg_json, meta); !e.empty()) return e;
    if (!meta.quantized) return "checkpoint has no quantization_config";

    if (meta.quant.quant_method == "exl3") {
        if (meta.arch == "qwen3_next" || meta.arch == "qwen3next")
            return import_exl3_qwen3next_to_gguf(hf_dir, out_gguf,
                                                 tokenizer_ref_gguf, log);
        return import_exl3_to_gguf(hf_dir, out_gguf, tokenizer_ref_gguf, log);
    }
    return import_awq_to_gguf(hf_dir, out_gguf, tokenizer_ref_gguf, log);
}
