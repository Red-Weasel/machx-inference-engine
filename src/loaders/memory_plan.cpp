...
// Shared footprint formatter (declared in the header) — the planner note and the
// load-time OOM gate both format bytes here so the byte→GB math lives in one place.
...
// Keep this list in lockstep with the arch backends that actually implement a
// split/TP forward (Engine::load and the preflight fit check both call this).
bool arch_can_multigpu(ModelArch arch) noexcept {
    return is_dense_arch(arch)                ||
           arch == ModelArch::kQwen35Dense    ||
           arch == ModelArch::kQwen3Next      ||
           arch == ModelArch::kGptOss         ||
           arch == ModelArch::kQwen35Moe      ||
           arch == ModelArch::kQwen3Moe;
}

bool should_block_oom(const PlacementPlan& plan, bool allow_oom_env) noexcept {
    // `forced` (user pinned --gpus) overrides only the planner's AUTO gpu-count
    // choice — NOT the fit gate. Forcing the card count and consenting to OOM are
    // different intents, so a forced-but-over-budget plan (e.g. a 222k KV that
    // can't fit on the pinned cards) still gets the actionable refusal instead of
    // a deep OOM crash. IE_ALLOW_OOM=1 is the sole "let me OOM anyway" escape hatch.
    return !allow_oom_env && !plan.fits;
}
...
uint32_t count_matching_gpus(std::string_view name_filter) {
    ...
    for (const auto& d : sycl::device::get_devices()) {
        ...
        if (name_filter.empty() ||
            ...
            ++n;
            if (d.get_backend() == sycl::backend::ext_oneapi_level_zero) ++level_zero;
        }
    }
    // Intel cards are exposed by both Level Zero and OpenCL. Multi-card Intel
    // runtimes use Level Zero; counting the OpenCL aliases doubles capacity.
    if (level_zero) return level_zero;
...
PlacementPlan plan_placement(const GgufReader& g, ModelArch /*arch*/,
    ...
    plan.ws_bytes       = kWorkspaceMarginBytes;
    plan.per_card_bytes = usable_card_vram(dev);
    plan.avail_gpus     = count_matching_gpus(name_filter);

    // Forced: the user pinned --gpus N — respect the COUNT (clamped to available), but
    // still compute `fits` so the OOM gate can refuse a forced-but-over-budget pin
    // (e.g. a 222k KV that won't fit on the pinned cards) instead of crashing deep in
    // the load. Forcing the card count is NOT consent to OOM — IE_ALLOW_OOM is. Unknown
    // VRAM (per_card_bytes==0) never blocks. Mirrors the non-forced fit math below.
    if (requested > 0) {
        plan.forced = true;
        ...
        plan.fits = (plan.per_card_bytes == 0) ||
                    (need <= uint64_t(plan.n_gpus) * plan.per_card_bytes);
