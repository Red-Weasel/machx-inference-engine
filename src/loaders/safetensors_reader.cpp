...
SafetensorsReader::SafetensorsReader(SafetensorsReader&& o) noexcept
    : fd_(o.fd_), dfd_(o.dfd_), base_(o.base_), size_(o.size_),
      tensors_(std::move(o.tensors_)), meta_(std::move(o.meta_)), index_(std::move(o.index_)) {
    o.fd_ = -1; o.dfd_ = -1; o.base_ = nullptr; o.size_ = 0;
}
...
void SafetensorsReader::close() {
    ...
    fd_ = -1; dfd_ = -1; base_ = nullptr; size_ = 0;
    tensors_.clear(); meta_.clear(); index_.clear();
}
...
DType SafetensorsReader::dtype_from_string(std::string_view s) {
    ...
    // DeepSeek-V4.1-Flash (2026-09-12). These carry no scales of their own — the scale
    // plane is a sibling tensor `<name>.scale` — so the DType names the element
    // encoding only. The FP4 expert planes are written as I8 with half the logical
    // width, indistinguishable from a real int8 tensor by dtype string alone, so they
    // are retyped by ROLE at bind time, not here.
    if (s == "F8_E4M3") return DType::kFP8_E4M3;
    if (s == "F8_E8M0") return DType::kE8M0;
    // F8_E5M2 / U16/U32/U64 left unmapped (kCount); dtype_str is kept either way.
    return DType::kCount;
}
...
bool SafetensorsReader::advise(int advice) const {
    return base_ && base_ != MAP_FAILED && ::madvise(base_, size_, advice) == 0;
}
...
std::string SafetensorsReader::open(const std::string& path) {
    ...
    dfd_ = ::open(path.c_str(), O_RDONLY | O_DIRECT);   // -1 is fine: readers fall back to fd_
    ...
    for (auto it = hdr.begin(); it != hdr.end(); ++it) {
        ...
        index_.emplace(key, tensors_.size());
...
bool SafetensorsModel::advise(int advice) const {
    bool ok = true;
    for (const auto& s : shards_) ok = s->advise(advice) && ok;
    return ok;
}

bool SafetensorsModel::locate(const uint8_t* p, int& fd, int& direct_fd, uint64_t& off) const {
    for (const auto& s : shards_)
        if (s->base() && p >= s->base() && p < s->base() + s->size()) {
            fd = s->fd(); direct_fd = s->direct_fd(); off = uint64_t(p - s->base()); return true;
        }
    return false;
}
