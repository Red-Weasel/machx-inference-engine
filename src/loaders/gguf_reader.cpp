...
std::string GgufReader::open(const std::string& path) {
    ...
    // NO madvise here, deliberately — the mapping keeps the kernel's default
    // readahead window.  This used to be MADV_RANDOM ("tensor data is touched
    // sparsely"), which was wrong for both halves of a GGUF:
    //   * the header (KV table + tensor-info table) is walked strictly
    //     front-to-back by parse_shard below, on every open, for every shard;
    //   * the tensor-data blob is streamed whole-tensor in ascending order by
    //     every model loader in the engine.
    // MADV_RANDOM zeroes the VMA's ra_pages, turning each fault into one
    // blocking 4 KiB read.  Measured 2026-08-02, cold page cache, DeepSeek-V4-
    // Flash UD-Q3_K_XL (4 shards / 120 GB) on a contended USB spinning disk —
    // a concurrent download held it at ~95% util, and a contemporaneous
    // dd bs=32M iflag=direct on the same file read 6.7 MB/s:
    //
    //                            with MADV_RANDOM    without (this code)
    //     4-shard header parse        58.1 s               1.3 s
    //     ascending tensor walk    0.0608 MB/s        12.7 - 14.7 MB/s
    //     iostat rareq-sz             4.00 kB           302 - 492 kB
    //     deepseek4_config_test      57.85 s               1.20 s
    //
    // Note which way round that goes: the metadata-only readers were the WORST
    // hit, not the least.  A header walk is pure sequential access, so turning
    // readahead off cost the "sparse" config-only path 48x.  The only cost of
    // the default window is a bounded over-read — the 4-shard open pulls 8.0 MB
    // off disk instead of 4.6 MB, and buys back 57 s.
    // MADV_WILLNEED is NOT a substitute: the readahead it forces is bounded by
    // the VMA's own ra_pages, so it does nothing while MADV_RANDOM stands.
    // tests/unit/gguf_madvise_test.cpp asserts VM_RAND_READ stays off.
    ...
    if (count > 1) {
        ...
        for (uint32_t i = 2; i <= count; ++i) {
            ...
            // Default readahead, as for the primary mapping above — a secondary
            // shard is header-parsed sequentially and then streamed, same as any
            // other. See the comment at the primary mmap for the measurements.
...
std::string GgufReader::parse_shard(const uint8_t* base, size_t size, bool primary) {
    ...
    // A metadata-only shard (GLM-5.3-Flash ships a 9 MB tensor_count=0 shard 1)
    // has no data section at all — the padded cursor may land past EOF and
    // that is fine. Only a shard that contributed tensors needs the check.
    if (tensors_.size() > first && data_off > size) return "tensor data starts past EOF";
...
bool GgufReader::locate(const void* p, int& fd, uint64_t& off) const noexcept {
    const uint8_t* u = static_cast<const uint8_t*>(p);
    if (base_ && u >= base_ && u < base_ + size_) { fd = fd_; off = uint64_t(u - base_); return true; }
    for (const auto& sh : extra_shards_)
        if (sh.base && u >= sh.base && u < sh.base + sh.size) { fd = sh.fd; off = uint64_t(u - sh.base); return true; }
    return false;
}
