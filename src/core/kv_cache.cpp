...
// One contiguous memcpy per (layer, kv_head, K|V) slot — the trailing
// two axes (pos, head_dim) are contiguous, so each copy is
// `length * head_dim * sizeof(half)` bytes.  No quantize step.
std::string KvCache::copy_prefix_from(sycl::queue& q,
    ...
    if (cfg_.use_int8 || src.cfg_.use_int8)
        return "copy_prefix_from: INT8 KV not yet supported";
