...
#include "ie/ops.hpp"   // dequantize_kv_from_int8 (int8 KV snapshot regen)
...
// PR #3 — copy a prefix of length `length` from `src` into this cache.
// Source and destination may have different `max_ctx` (the prefix-cache
// pattern: small "stored prefix" copies into a full-size run-time cache).
// fp16 mode: one contiguous memcpy per (layer, kv_head, K|V) slot — the trailing
// two axes (pos, head_dim) are contiguous, so each copy is
// `length * head_dim * sizeof(half)` bytes.
// int8 mode: snapshot the int8 shadow + per-row scales, then regenerate fp16 from
// them (the int8 shadow is the authoritative history in --int8-kv). Both caches
// must agree on use_int8.
std::string KvCache::copy_prefix_from(sycl::queue& q,
                                      const KvCache& src, uint32_t length) {
    if (!ready() || !src.ready()) return "copy_prefix_from: cache not ready";
    if (cfg_.n_layers_full != src.cfg_.n_layers_full)
        return "copy_prefix_from: n_layers_full mismatch";
    if (cfg_.n_kv_heads != src.cfg_.n_kv_heads)
        return "copy_prefix_from: n_kv_heads mismatch";
    if (cfg_.head_dim != src.cfg_.head_dim)
        return "copy_prefix_from: head_dim mismatch";
    // INT8 snapshot support: both caches must agree on the mode. A fp16↔int8
    // copy is meaningless (different backing storage), so reject a mismatch.
    if (cfg_.use_int8 != src.cfg_.use_int8)
        return "copy_prefix_from: use_int8 mismatch";
    if (length > src.cfg_.max_ctx) return "copy_prefix_from: length > src.max_ctx";
    if (length > cfg_.max_ctx)     return "copy_prefix_from: length > dst.max_ctx";

    const uint64_t d_head = cfg_.head_dim;
    const uint64_t n_kv_h = cfg_.n_kv_heads;

    if (!cfg_.use_int8) {
        // ---- fp16 path: one contiguous memcpy per (layer, kv_head, K|V) slot.
        const uint64_t per_layer_src = n_kv_h * uint64_t(src.cfg_.max_ctx) * d_head;
        const uint64_t per_layer_dst = n_kv_h * uint64_t(cfg_.max_ctx)     * d_head;
        const uint64_t bytes_per_kvh = uint64_t(length) * d_head * sizeof(sycl::half);

        std::vector<sycl::event> evts;
        evts.reserve(cfg_.n_layers_full * n_kv_h * 2);

        for (uint32_t L = 0; L < cfg_.n_layers_full; ++L) {
            for (uint32_t kv = 0; kv < n_kv_h; ++kv) {
                const uint64_t src_off = L * per_layer_src + uint64_t(kv) * uint64_t(src.cfg_.max_ctx) * d_head;
                const uint64_t dst_off = L * per_layer_dst + uint64_t(kv) * uint64_t(cfg_.max_ctx)     * d_head;
                evts.push_back(q.memcpy(k_ + dst_off, src.k_ + src_off, bytes_per_kvh));
                evts.push_back(q.memcpy(v_ + dst_off, src.v_ + src_off, bytes_per_kvh));
            }
            lengths_[L] = length;  // host-side metadata
        }
        // Wait so caller can rely on lengths_ being authoritative when this returns.
        for (auto& e : evts) e.wait();
        return {};
    }

    // ---- INT8 path: the int8 shadow (k_int8_/v_int8_) + per-row fp16 scales
    // (k_scales_/v_scales_) are authoritative up to int8_length; the fp16 K/V is
    // only current where a T>1 kernel last wrote it. So snapshot the int8 shadow
    // + scales, then REGENERATE fp16 from them (both this cache's own buffers).
    // Doing the dequant for BOTH copy directions (snapshot-insert dst=snapshot,
    // restore dst=runtime) makes fp16 always derive from int8 — it never depends
    // on which positions happened to be T>1-touched.
    //   int8 data element-offset == fp16 element-offset (same [L,kv,ctx,d] layout,
    //   int8 = 1 B/elem, half = 2 B/elem); scales are [L,kv,ctx], stride max_ctx.
    const uint64_t per_layer_i8_src = n_kv_h * uint64_t(src.cfg_.max_ctx) * d_head;
    const uint64_t per_layer_i8_dst = n_kv_h * uint64_t(cfg_.max_ctx)     * d_head;
    const uint64_t per_layer_sc_src = n_kv_h * uint64_t(src.cfg_.max_ctx);
    const uint64_t per_layer_sc_dst = n_kv_h * uint64_t(cfg_.max_ctx);
    const uint64_t i8_bytes_per_kvh = uint64_t(length) * d_head * sizeof(int8_t);
    const uint64_t sc_bytes_per_kvh = uint64_t(length) * sizeof(sycl::half);

    std::vector<sycl::event> evts;
    evts.reserve(cfg_.n_layers_full * n_kv_h);

    for (uint32_t L = 0; L < cfg_.n_layers_full; ++L) {
        for (uint32_t kv = 0; kv < n_kv_h; ++kv) {
            const uint64_t i8_src_off = L * per_layer_i8_src + uint64_t(kv) * uint64_t(src.cfg_.max_ctx) * d_head;
            const uint64_t i8_dst_off = L * per_layer_i8_dst + uint64_t(kv) * uint64_t(cfg_.max_ctx)     * d_head;
            const uint64_t sc_src_off = L * per_layer_sc_src + uint64_t(kv) * uint64_t(src.cfg_.max_ctx);
            const uint64_t sc_dst_off = L * per_layer_sc_dst + uint64_t(kv) * uint64_t(cfg_.max_ctx);
            // Snapshot int8 + scales into THIS cache.
            sycl::event e0 = q.memcpy(k_int8_   + i8_dst_off, src.k_int8_   + i8_src_off, i8_bytes_per_kvh);
            sycl::event e1 = q.memcpy(v_int8_   + i8_dst_off, src.v_int8_   + i8_src_off, i8_bytes_per_kvh);
            sycl::event e2 = q.memcpy(k_scales_ + sc_dst_off, src.k_scales_ + sc_src_off, sc_bytes_per_kvh);
            sycl::event e3 = q.memcpy(v_scales_ + sc_dst_off, src.v_scales_ + sc_src_off, sc_bytes_per_kvh);
            // Regenerate fp16 from THIS cache's just-copied int8+scales.
            evts.push_back(dequantize_kv_from_int8(
                q, k_int8_ + i8_dst_off, v_int8_ + i8_dst_off,
                k_scales_ + sc_dst_off, v_scales_ + sc_dst_off,
                k_ + i8_dst_off, v_ + i8_dst_off,
                length, uint32_t(d_head), {e0, e1, e2, e3}));
        }
        lengths_[L]      = length;  // fp16 valid up to length (regenerated above)
        int8_lengths_[L] = length;  // int8 shadow valid up to length
    }
    // Wait so caller can rely on lengths_/int8_lengths_ + regenerated fp16 being
    // authoritative when this returns (dequant events transitively cover memcpys).
    for (auto& e : evts) e.wait();
    return {};
}
