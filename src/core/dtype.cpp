...
constexpr std::array<TypeInfo, static_cast<size_t>(DType::kCount)> kTable = {{
    ...
    { DType{40},       "<reserved>",   0,    0,   false },
    // Prism ternary quants (llama.cpp-prism fork, ggml_type 41/42).
    { DType::kQ1_0,    "Q1_0",       128,   18,   true  },
    { DType::kQ2_0,    "Q2_0",       128,   34,   true  },
    // Tencent Hy4-preview scaled-ternary (hy4-preview llama.cpp patch, type 43).
    { DType::kSTQ1_0,  "STQ1_0",     256,   42,   true  },
}};

// ik_llama.cpp extended types (ggml ids 128+). Kept sparse: padding the dense
// array out to id 153 would add ~110 dead rows. Sizes were derived from the
// GLM-5.2 GGUF's own consecutive tensor offsets and then confirmed against
// ik_llama.cpp's static_asserts — see docs/glm52/KERNEL_VALIDATION.md §1.
//
//                   { dtype,        name,    block, bytes, quantized?, row_meta }
constexpr std::array<TypeInfo, 5> kTableExt = {{
    { DType::kQ6_0,    "Q6_0",         32,   26,   true,  0 },
    { DType::kIQ2_KT,  "IQ2_KT",      256,   68,   true,  4 },
    // safetensors element types (engine-custom, 200+). These carry NO scales of
    // their own — the scale plane is a sibling tensor — so `block_size` here is
    // the packing ratio, not a quantisation group, and `bytes_for` stays exact:
    // FP4_E2M1 is 2 elements to the byte, the other two are 1 byte each.
    { DType::kFP8_E4M3, "FP8_E4M3",     1,    1,   true,  0 },
    { DType::kFP4_E2M1, "FP4_E2M1",     2,    1,   true,  0 },
    { DType::kE8M0,     "E8M0",         1,    1,   false, 0 },
}};
...
const TypeInfo* type_info(DType d) noexcept {
    ...
    if (i >= kTable.size()) {
        for (const auto& ti : kTableExt)
            if (ti.dtype == d) return &ti;
        return nullptr;
    }
...
size_t bytes_for(DType d, size_t n_elements) noexcept {
    ...
    // Every caller passes ONE row's element count and multiplies by the row
    // count itself, so charging the per-row meta here is both correct and the
    // only place it needs to happen. IQ2_KT: 4 + (n/256)*68.
    return blocks * ti->block_bytes + ti->row_meta_size;
}
