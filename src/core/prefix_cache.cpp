...
std::string PrefixCache::insert(sycl::queue& q,
    ...
    {
        // Inherit the live cache's config (esp. use_int8) so the snapshot mode
        // matches — otherwise copy_prefix_from rejects a fp16↔int8 mismatch. Only
        // max_ctx differs (the snapshot needs just `tokens.size()` depth).
        KvCacheConfig kvcfg = kv.config();
