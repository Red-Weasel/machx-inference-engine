...
Built `llama-cli` from `~/llama.cpp` (Qwen3.5-MoE arch already supported there as `LLM_ARCH_QWEN35MOE`). Source review of `src/models/qwen35moe.cpp` and `convert_hf_to_gguf.py:_LinearAttentionVReorderBase` revealed 3 silent-correctness bugs:
...
- `tools/llama_dump.cpp` — links against `~/llama.cpp/build-vk/bin/{libllama,libggml,libggml-base}`. Registers `cb_eval` to capture per-layer residual stream (`<prefix>_L00..L41.bin`) and per-block attn output (`<prefix>_A00..A39.bin`) as fp32 binaries. Each file has a sidecar `.meta` with text "T H".
